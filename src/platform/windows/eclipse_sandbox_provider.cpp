/**
 * @file src/platform/windows/eclipse_sandbox_provider.cpp
 * @brief Fail-closed Windows process provider for Eclipse sandboxes.
 */

// header include
#include "eclipse_sandbox_provider.h"
...
namespace eclipse::windows::sandbox {
  namespace {
     ...
     */
    bool validate_application(const eclipse_sandboxes::application_t &application, std::string &reason) {
     ...
     */
    bool capable(const eclipse_sandboxes::launch_request_t &request, const bool has_secret_resolver, std::string &reason) {
      reason.clear();
      if (!supported_platform()) {
        return reject(reason, "Windows sandbox provider requires Windows 10 22H2 or newer on x64");
      }
      json normalized;
      if (!eclipse_sandboxes::normalize_policy(request.effective_policy, normalized) || normalized != request.effective_policy) {
    ...
    struct provider_t {
       ...
       */
      std::optional<eclipse_sandboxes::launch_result_t> launch(const eclipse_sandboxes::launch_request_t &request, eclipse_sandboxes::error_t &error) {
        std::string reason;
        ...
        return eclipse_sandboxes::launch_result_t {runtime_id, std::nullopt, {}, {}};
      }
       ...
       */
      eclipse_sandboxes::termination_t terminate(const std::string &runtime_id, const bool force) {
        ...
        if (!active_processes(runtime->job.get(), active)) {
          return {false, std::nullopt, eclipse_sandboxes::error_t {"job_query_failed", "Sandbox Job Object state could not be queried"}};
        }
        if (active != 0 && !TerminateJobObject(runtime->job.get(), ERROR_PROCESS_ABORTED)) {
          return {false, std::nullopt, eclipse_sandboxes::error_t {"termination_failed", "Sandbox Job Object could not be terminated"}};
        }
        const auto deadline = GetTickCount64() + TERMINATION_WAIT_MS;
        do {
          if (!active_processes(runtime->job.get(), active)) {
            return {false, std::nullopt, eclipse_sandboxes::error_t {"job_query_failed", "Sandbox termination could not be confirmed"}};
          }
          if (active == 0) {
            break;
          }
        ...
        if (active != 0) {
          return {false, std::nullopt, eclipse_sandboxes::error_t {"termination_timeout", "Sandbox processes did not terminate in time"}};
        }
       ...
       */
      eclipse_sandboxes::reconciliation_t reconcile(const std::string &runtime_id) {
        std::shared_ptr<runtime_t> runtime;
        {
          std::scoped_lock lock {mutex};
          const auto found = runtimes.find(runtime_id);
          if (found == runtimes.end()) {
            return {eclipse_sandboxes::reconciliation_t::status_t::missing, std::nullopt, std::nullopt};
          }
          runtime = found->second;
        }
        DWORD active = 0;
        if (!active_processes(runtime->job.get(), active)) {
          return {eclipse_sandboxes::reconciliation_t::status_t::error, std::nullopt, eclipse_sandboxes::error_t {"job_query_failed", "Sandbox Job Object state could not be queried"}};
        }
        if (active != 0) {
          return {eclipse_sandboxes::reconciliation_t::status_t::running, std::nullopt, std::nullopt};
        }
        runtime->stop_watchdog();
        const auto exit_code = process_exit_code(runtime->process.get());
        {
        ...
        return {eclipse_sandboxes::reconciliation_t::status_t::exited, exit_code, std::nullopt};
      }
    };
  }  // namespace
  ...
  eclipse_sandboxes::callbacks_t make_callbacks(configuration_t configuration) {
    ...
    return {
      ...
      [provider](const eclipse_sandboxes::launch_request_t &request, std::string &reason) {
        return capable(request, static_cast<bool>(provider->configuration.resolve_secret), reason);
      },
      [provider](const eclipse_sandboxes::launch_request_t &request, eclipse_sandboxes::error_t &error) {
