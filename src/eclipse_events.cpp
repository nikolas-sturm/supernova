/**
 * @file src/eclipse_events.cpp
 * @brief Thread-safe Eclipse V1 event replay hub implementation.
 */
...
namespace eclipse_events {
  ...
  hub_t::hub_t(const std::size_t replay_capacity, clock_t clock):
    ...
    if (replay_capacity == 0 || !clock) {
      throw std::invalid_argument("Eclipse event replay capacity and clock must be valid");
    }
  ...
  void hub_t::publish(event_t event, const std::vector<std::string> &recipients) {
    if (!valid_type(event.type)) {
      throw std::invalid_argument("Unsupported Eclipse event type");
    }
  ...
  open_result_t hub_t::open(const std::string &client_id, const std::optional<std::string> &last_event_id, std::vector<std::string> collections) {
    if (client_id.empty()) {
      throw std::invalid_argument("Eclipse event client identity must not be empty");
    }
    std::scoped_lock lock {impl_->mutex};
