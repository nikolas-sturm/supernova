...
int main() {
    ...
    expect(eclipse::displayModeMatchesFrameRate(119, 60),
           "Rounded refresh rate did not match stream cadence.");
    expect(!eclipse::displayModeMatchesFrameRate(144, 60),
           "Non-divisible refresh rate matched stream cadence.");
    expect(!eclipse::displayModeMatchesFrameRate(61, 60),
           "Higher mismatched refresh rate was treated as a fractional alias.");
    expect(eclipse::selectOptimalDisplayMode(modes, {3840, 2160, 60}, 1920, 1080, 60) ==
               DisplayModeSpec{3840, 2160, 120},
           "Highest cadence-matched desktop mode was not selected.");
    expect(eclipse::selectOptimalDisplayMode(modes, {2560, 1600, 60}, 1920, 1080, 120) ==
               DisplayModeSpec{1920, 1080, 120},
           "Closest lower-resolution aspect match was not selected.");
    expect(eclipse::selectOptimalDisplayMode(modes, {3840, 2160, 60}, 1920, 1080, 100) ==
