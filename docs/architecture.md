...
Sol is the native streaming host with a React administration frontend.
Terra is the streaming client: Neutralino hosts its React UI, while its native
...
Shared contracts have **not** yet been extracted. Sol's administration API
...
`dark` is the acid/violet Terra default. `auto` is an additional preference that
...
Consult [Sol's license](../apps/sol/LICENSE) and
