/**
 * @file tests/unit/test_eclipse_api.cpp
 * @brief Test Eclipse API capability and discovery metadata.
 */
...
TEST(EclipseApiTest, DerivesRfc6455AcceptKey) {
  EXPECT_EQ(
...
TEST(EclipseApiTest, RoundTripsWebSocketFrames) {
...
TEST(EclipseApiTest, SerializesOperationalCapabilitiesForServerInfo) {
  EXPECT_EQ(
    eclipse_api::capabilities_csv(),
    "client-permissions,session-ids,structured-errors,catalog-v2,events-v1,profiles-v1,workspaces-v1,telemetry-v1"
  );
}

TEST(EclipseApiTest, RequiresCompleteExplicitPairingPolicy) {
  EXPECT_FALSE(eclipse_api::parse_pairing_policy("catalog.read", std::nullopt).valid);
  EXPECT_FALSE(eclipse_api::parse_pairing_policy(std::nullopt, "keyboard").valid);
  EXPECT_FALSE(eclipse_api::parse_pairing_policy("unknown.scope", "keyboard").valid);
  EXPECT_FALSE(eclipse_api::parse_pairing_policy("catalog.read", "unknown-input").valid);
}

TEST(EclipseApiTest, ParsesExplicitPairingPolicyWithoutImplicitGrants) {
  const auto policy = eclipse_api::parse_pairing_policy("catalog.read,session.control", "keyboard,controller");
  ...
  const auto empty_policy = eclipse_api::parse_pairing_policy("", "");
...
TEST(EclipseApiTest, PreservesLegacyPairingPolicyWhenFieldsAreAbsent) {
  const auto policy = eclipse_api::parse_pairing_policy(std::nullopt, std::nullopt);

  ASSERT_TRUE(policy.valid);
  ASSERT_FALSE(policy.explicit_policy);
  EXPECT_EQ(policy.permissions.scopes.size(), eclipse_api::SCOPES.size());
  EXPECT_TRUE(policy.permissions.input.keyboard);
...
TEST(EclipseApiTest, RecognizesOnlyExplicitApiV1OptIn) {
  EXPECT_TRUE(eclipse_api::api_v1_requested("1"));
  EXPECT_FALSE(eclipse_api::api_v1_requested(""));
  EXPECT_FALSE(eclipse_api::api_v1_requested("2"));
}

TEST(EclipseApiTest, ValidatesWakeOnLanAddresses) {
  EXPECT_TRUE(eclipse_api::wake_on_lan_available("02:00:5e:10:00:00"));
  EXPECT_TRUE(eclipse_api::wake_on_lan_available("AA:BB:CC:DD:EE:FF"));
  EXPECT_FALSE(eclipse_api::wake_on_lan_available("00:00:00:00:00:00"));
  EXPECT_FALSE(eclipse_api::wake_on_lan_available("01:00:5e:00:00:01"));
  EXPECT_FALSE(eclipse_api::wake_on_lan_available("not-a-mac"));
  EXPECT_FALSE(eclipse_api::wake_on_lan_available("00-11-22-33-44-55"));
}

TEST(EclipseApiTest, BuildsCompleteCapabilityDocument) {
  ...
  EXPECT_EQ(document["host"]["name"], "Sunshine host");
  ...
  EXPECT_EQ(document["features"].size(), eclipse_api::KNOWN_CAPABILITIES.size());
  ...
  EXPECT_GE(document["capabilities"].size(), eclipse_api::CAPABILITIES.size());
...
#else
  EXPECT_EQ(document["capabilities"].size(), eclipse_api::CAPABILITIES.size());
...
TEST(EclipseApiTest, BuildsCatalogResourceWithoutUnsupportedProfileLinks) {
  const auto image_path = std::filesystem::path {SUNSHINE_TEST_BIN_DIR} / "eclipse-catalog-resource.png";
...
TEST(EclipseApiTest, CatalogDefaultsDoNotDependOnActiveEncoderState) {
  proc::ctx_t app;
  app.uuid = "11111111-1111-1111-1111-111111111111";
  app.id = "42";
  app.name = "Example";
  app.image_path = "missing-eclipse-catalog-image.png";
  app.eclipse_metadata = nlohmann::json::object();
...
TEST(EclipseApiTest, BoundsRequestBodyBeforeMaterializingOverflow) {
  std::istringstream exact {"12345678"};
  auto accepted = eclipse_api::read_bounded_body(exact, 8);
  EXPECT_EQ(accepted.status, eclipse_api::body_read_status_t::success);
  EXPECT_EQ(accepted.text, "12345678");

  std::istringstream oversized {"123456789"};
  auto rejected = eclipse_api::read_bounded_body(oversized, 8);
  EXPECT_EQ(rejected.status, eclipse_api::body_read_status_t::too_large);
  EXPECT_TRUE(rejected.text.empty());
}

TEST(EclipseApiTest, ProjectsEventCollectionsFromReadScopes) {
  eclipse_api::client_permissions_t permissions;
