/**
 * @file tests/unit/test_eclipse_peripherals.cpp
 * @brief Unit tests for Eclipse peripherals-v1 device registry and claim manager.
 */
// standard includes
#include <chrono>
#include <string>
...
// local includes
#include <src/eclipse_peripherals.h>
...
namespace {
   ...
   */
  eclipse_peripherals::manager_t make_manager() {
    return eclipse_peripherals::manager_t(test_now, test_uuid, test_token);
  }
   ...
   */
  eclipse_peripherals::create_device_t keyboard_device() {
    return {
      .device_class = "keyboard",
      .platform_id = "hid-keyboard-1",
      .name = "Eclipse Keyboard",
      .vendor_id = 0x1234,
      .product_id = 0x5678,
      .serial = std::nullopt,
   ...
   * @return Creation fields.
   */
  eclipse_peripherals::create_claim_t session_claim(const std::string &device_id) {
    return {
      .device_id = device_id,
  ...
  TEST(EclipsePeripheralsTest, RejectsUnsupportedClassesAndCapabilities) {
    auto manager = make_manager();
    auto unsupported_class = keyboard_device();
    unsupported_class.device_class = "usb";
    EXPECT_EQ(manager.create_device("owner-1", unsupported_class).status, eclipse_peripherals::status_t::unsupported_configuration);

    auto unsupported_capability = keyboard_device();
    unsupported_capability.capabilities = {"usb.bulk"};
    EXPECT_EQ(manager.create_device("owner-1", unsupported_capability).status, eclipse_peripherals::status_t::unsupported_configuration);

    auto mismatched_capability = keyboard_device();
    mismatched_capability.capabilities = {"mouse.hid"};
    EXPECT_EQ(manager.create_device("owner-1", mismatched_capability).status, eclipse_peripherals::status_t::unsupported_configuration);

    auto duplicate_capabilities = keyboard_device();
    duplicate_capabilities.capabilities = {"keyboard.hid", "keyboard.hid"};
    EXPECT_EQ(manager.create_device("owner-1", duplicate_capabilities).status, eclipse_peripherals::status_t::unsupported_configuration);
  }

  TEST(EclipsePeripheralsTest, RegistersAndListsOwnedDevices) {
    auto manager = make_manager();
    const auto created = manager.create_device("owner-1", keyboard_device());
    ASSERT_EQ(created.status, eclipse_peripherals::status_t::success);
    ...
    const auto serialized = eclipse_peripherals::device_json(*created.resource);
    EXPECT_EQ(serialized.at("class"), "keyboard");
    EXPECT_TRUE(serialized.at("claimable"));
    EXPECT_TRUE(serialized.at("activeClaimId").is_null());
  }

  TEST(EclipsePeripheralsTest, ClaimLifecycleEnforcesVisibilityAndExclusivity) {
    auto manager = make_manager();
    const auto device = manager.create_device("owner-1", keyboard_device());
    ASSERT_EQ(device.status, eclipse_peripherals::status_t::success);
    ...
    ASSERT_EQ(claim.status, eclipse_peripherals::status_t::success);
    ASSERT_TRUE(claim.resource);
    EXPECT_EQ(claim.resource->state, eclipse_peripherals::claim_state_t::pending);
    EXPECT_EQ(claim.resource->device_class, "keyboard");

    // Same owner may not double-claim exclusively.
    EXPECT_EQ(manager.create_claim("owner-1", claim_request).status, eclipse_peripherals::status_t::conflict);

    // Foreign owner cannot see or claim the device.
    EXPECT_TRUE(manager.list_devices("owner-2").empty());
    EXPECT_EQ(manager.create_claim("owner-2", claim_request).status, eclipse_peripherals::status_t::not_found);
    ...
    ASSERT_EQ(opened.status, eclipse_peripherals::status_t::success);
    EXPECT_EQ(opened.resource->state, eclipse_peripherals::claim_state_t::active);
    ...
    ASSERT_TRUE(closed);
    EXPECT_EQ(closed->state, eclipse_peripherals::claim_state_t::released);

    // Release is idempotent.
    const auto released = manager.release_claim("owner-1", claim.resource->id);
    ASSERT_EQ(released.status, eclipse_peripherals::status_t::success);
    const auto released_again = manager.release_claim("owner-1", claim.resource->id);
    ASSERT_EQ(released_again.status, eclipse_peripherals::status_t::success);
    EXPECT_EQ(released.resource->state, released_again.resource->state);
  }

  TEST(EclipsePeripheralsTest, TargetEndHonorsDisconnectPolicy) {
    auto manager = make_manager();
    const auto device = manager.create_device("owner-1", keyboard_device());
    ASSERT_EQ(device.status, eclipse_peripherals::status_t::success);
    ...
    ASSERT_EQ(suspend_claim.status, eclipse_peripherals::status_t::success);
    ...
    EXPECT_EQ(manager.get_claim("owner-1", suspend_claim.resource->id)->state, eclipse_peripherals::claim_state_t::suspended);
    ...
    ASSERT_EQ(release_claim.status, eclipse_peripherals::status_t::success);
    ...
    EXPECT_EQ(manager.get_claim("owner-1", release_claim.resource->id)->state, eclipse_peripherals::claim_state_t::released);
  }

  TEST(EclipsePeripheralsTest, DeleteDeviceReleasesClaimsAndRevocationClearsOwner) {
    auto manager = make_manager();
    const auto device = manager.create_device("owner-1", keyboard_device());
    ASSERT_EQ(device.status, eclipse_peripherals::status_t::success);
    ...
    EXPECT_EQ(manager.get_claim("owner-1", claim.resource->id)->state, eclipse_peripherals::claim_state_t::released);

    const auto removed = manager.delete_device("owner-1", device.resource->id);
    ASSERT_EQ(removed.status, eclipse_peripherals::status_t::success);
    EXPECT_TRUE(manager.list_devices("owner-1").empty());
    EXPECT_EQ(manager.delete_device("owner-1", device.resource->id).status, eclipse_peripherals::status_t::not_found);

    const auto serialized = eclipse_peripherals::claim_json(*claim.resource);
