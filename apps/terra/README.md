...
V-Sync, stereo/5.1/7.1 audio, host audio mute, Sunshine game optimizations, connection warnings,
...
persisted only after a compatible Sunshine `serverinfo` response succeeds. Blocked-port diagnostics
...
Surround output is preflighted against the default audio endpoint before Sunshine launch. Unsupported
5.1 or 7.1 layouts downgrade to stereo instead of starting a silent stream.

Ending a stream stops Moonlight transport and leaves the Sunshine app available to resume by
...
and Sunshine HDR metadata reach compatible compositors and displays. Vulkan presentation composites
...
On Linux, SDL2 controllers also expose Sunshine touchpads, host-requested accelerometer and gyroscope
...
Sunshine touch input with mouse fallback for older hosts. Windows also forwards native pen pressure,
...
Wayland, the stream overlay requires a compositor with wlr layer-shell support; Eclipse opens a native
...
never exposed to the webview. Eclipse sends bounded UDP port 9 broadcasts and rate-limits requests.

Before transport startup, Eclipse inspects the operating system route to the host. Native LAN routes
