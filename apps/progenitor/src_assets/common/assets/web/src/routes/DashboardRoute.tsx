...
import { SunshineVersion } from '../lib/version'
import {
  logsQuery,
  releaseQuery,
...
function VersionCard({ installedVersion, notifyPreReleases }: VersionCardProps) {
  const { t } = useTranslation()
  const installed = useMemo(() => new SunshineVersion(installedVersion), [installedVersion])
  ...
  const githubVersion = latest.data ? new SunshineVersion(latest.data.tag_name) : null
  ...
  const preReleaseAvailable =
    ...
    new SunshineVersion(preReleaseVersion.tag_name).isGreater(githubVersion)
