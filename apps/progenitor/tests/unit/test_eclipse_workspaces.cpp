/**
 * @file tests/unit/test_eclipse_workspaces.cpp
 * @brief Tests for standalone Eclipse workspace core.
 */
...
// local includes
#include <src/eclipse_workspaces.h>

namespace {
  using namespace eclipse_workspaces;
...
TEST(EclipseWorkspacesTest, ParsesExactWireRequests) {
...
TEST(EclipseWorkspacesTest, PublishesEveryDurableLifecycleTransition) {
  fake_t fake;
...
TEST(EclipseWorkspacesTest, CreatesExactDefinitionAndCollectionSnapshot) {
  fake_t fake;
...
TEST(EclipseWorkspacesTest, ValidatesCanonicalIdsPoliciesAndCallbacks) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
...
TEST(EclipseWorkspacesTest, PatchUsesCompleteReplacementsAndRollsBackFailedSave) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
  patch_t patch;
...
TEST(EclipseWorkspacesTest, StartPreparesReadyWorkspaceWithoutMediaSession) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
...
TEST(EclipseWorkspacesTest, FailedPreparationCleansPartialActualResources) {
  fake_t fake;
...
TEST(EclipseWorkspacesTest, FailedReadyPublicationCleansPreparedResources) {
  fake_t fake;
  int saves = 0;
  auto callbacks = fake.callbacks();
...
TEST(EclipseWorkspacesTest, FailedStartRollbackSaveDisablesManager) {
  fake_t fake;
  int saves = 0;
  auto callbacks = fake.callbacks();
  callbacks.save = [&](const std::string &value) {
...
TEST(EclipseWorkspacesTest, StopDisconnectsOrTerminatesWithExpectedResourceBehavior) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
  ASSERT_EQ(manager.start(WORKSPACE, 1).status, status_t::success);
...
TEST(EclipseWorkspacesTest, FailedStopAndPersistencePreservePublishedState) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
  ASSERT_EQ(manager.start(WORKSPACE, 1).status, status_t::success);
...
TEST(EclipseWorkspacesTest, FailedStopRestorationDisablesManager) {
  fake_t fake;
  int saves = 0;
  auto callbacks = fake.callbacks();
  callbacks.save = [&](const std::string &value) {
...
TEST(EclipseWorkspacesTest, RevocationStopsAndOrphansPersistentButDeletesEphemeral) {
...
TEST(EclipseWorkspacesTest, MultiResourceRevokeRestoresEarlierRuntimeAfterLaterStopFailure) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
...
TEST(EclipseWorkspacesTest, RevokeSaveFailureRestoresEveryStoppedRuntime) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
  ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
...
TEST(EclipseWorkspacesTest, RestartReconcilesRuntimeAndIsolatesMalformedRecords) {
  fake_t fake;
  {
    manager_t manager {fake.callbacks()};
    ASSERT_EQ(manager.create(OWNER, definition()).status, status_t::success);
...
TEST(EclipseWorkspacesTest, PersistentDefinitionsSurviveRestartAndEphemeralDoNot) {
  fake_t persistent_fake;
  {
...
TEST(EclipseWorkspacesTest, DeleteAndMutationsEnforceRevisionAndLifecycle) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
...
TEST(EclipseWorkspacesTest, MalformedDocumentFailsClosedAndInitialSaveFailureUnavailable) {
  fake_t malformed;
