/**
 * @file tests/unit/test_eclipse_profiles.cpp
 * @brief Tests for standalone Eclipse profile storage and validation.
 */
...
// local includes
#include <src/eclipse_profiles.h>

namespace {
  using eclipse::profiles::actor_t;
  using eclipse::profiles::callbacks_t;
  using eclipse::profiles::manager_t;
  ...
  TEST(EclipseProfiles, CreateGetListPatchDeleteAndRevisions) {
    std::string saved;
  ...
  TEST(EclipseProfiles, RejectsUnknownInvalidAndUnsupportedConfiguration) {
    auto cb = callbacks();
    cb.provider_supports = [](const auto &, const auto &) {
      return false;
    };
  ...
  TEST(EclipseProfiles, RequiresDomainScopeEvenForHostAdministrator) {
  ...
  TEST(EclipseProfiles, DisplayModeIdIsOpaqueNonEmptyText) {
    manager_t manager(callbacks());
    auto config = display();
  ...
  TEST(EclipseProfiles, LaunchReferencesRequireMatchingVisibleProfileType) {
    auto cb = callbacks();
  ...
  TEST(EclipseProfiles, FailedWriteRollsBackAndBusyBlocksDelete) {
    auto cb = callbacks();
    cb.save = [](const auto &) {
      return false;
    };
  ...
  TEST(EclipseProfiles, RecoversValidRecordsAndIsolatesMalformedOptionalRecord) {
    auto cb = callbacks();
    cb.load = [] {
  ...
  TEST(EclipseProfiles, PersistedRecordsRequireExactTypedFieldsAndFirstDuplicateWins) {
  ...
  TEST(EclipseProfiles, CanonicalSandboxNormalizerRejectsUnlistedEnvironmentValue) {
    manager_t manager(callbacks());
  ...
  TEST(EclipseProfiles, ProviderCanRejectDisplayTopologyAndVirtualDisplays) {
    auto cb = callbacks();
  ...
  TEST(EclipseProfiles, ProviderCanRejectLaunchPathsAndActions) {
    auto cb = callbacks();
    cb.provider_supports = [](const std::string &type, const json &) {
      return type != "launch";
    };
  ...
  TEST(EclipseProfiles, SandboxDefaultsAreReturnedAndSecretsRemainOpaque) {
    manager_t manager(callbacks());
  ...
  TEST(EclipseProfiles, SandboxAllowedApplicationsMustResolve) {
    auto cb = callbacks();
  ...
  TEST(EclipseProfiles, VisibilityAndAdoptionEnforceOwnership) {
    auto cb = callbacks();
    cb.load = [] {
  ...
  TEST(EclipseProfiles, MutationsMaskProfilesOwnedByAnotherClient) {
    manager_t manager(callbacks());
  ...
  TEST(EclipseProfiles, RevocationOrphansOwnedProfilesAtomically) {
    manager_t manager(callbacks());
    ASSERT_EQ(manager.create(admin(), {{"type", "stream"}, {"name", "shared"}, {"shared", true}, {"configuration", stream()}}).status, status_t::success);
  ...
  TEST(EclipseProfiles, PolicyReplacementOrphansOnlyUnauthorizedProfiles) {
    auto cb = callbacks();
    unsigned generated = 0;
    cb.uuid = [&generated] {
      return generated++ == 0 ? std::string {DISPLAY_PROFILE} : std::string {PROFILE};
  ...
  TEST(EclipseProfiles, LaunchProfileReferencesScanEveryStoredProfile) {
    auto cb = callbacks();
