...
%global branch 0
%global commit 0
%global sol_python_version 3.14
...
mkdir -p %{_builddir}/Sol
tar -xzf %{SOURCE0} -C %{_builddir}/Sol
...
cmake_args=(
  ...
  "-DSOL_ASSETS_DIR=%{_datadir}/sol"
  "-DSOL_EXECUTABLE_PATH=%{_bindir}/sol"
  "-DSOL_ENABLE_DRM=ON"
  "-DSOL_ENABLE_KWIN=ON"
  "-DSOL_ENABLE_PORTAL=ON"
  "-DSOL_ENABLE_WAYLAND=ON"
  "-DSOL_ENABLE_X11=ON"
  "-DSOL_PUBLISHER_NAME=LizardByte"
  "-DSOL_PUBLISHER_WEBSITE=https://app.lizardbyte.dev"
  "-DSOL_PUBLISHER_ISSUE_URL=https://app.lizardbyte.dev/support"
)

%if 0%{?fedora}
# uv installs Python and glad's Python dependencies into .venv before CMake runs.
cmake_args+=("-DGLAD_SKIP_PIP_INSTALL=ON")
cmake_args+=("-DPython_EXECUTABLE=%{_builddir}/Sol/.venv/bin/python")
%endif
...
%if 0%{?sle_version}
cmake_args+=("-DSOL_ENABLE_VULKAN=OFF")
%endif

# cmake
cd %{_builddir}/Sol
%if 0%{?fedora}
uv python install %{sol_python_version}
uv sync \
  --locked \
  --only-group glad \
  --python %{sol_python_version} \
  --no-install-project
%endif
echo "cmake args:"
echo "${cmake_args[@]}"
cmake "${cmake_args[@]}"
make -j$(nproc) -C "%{_builddir}/Sol/build"
...
# run tests
cd %{_builddir}/Sol/build
xvfb-run ./tests/test_sol
...
%files
# Executables
%caps(cap_sys_admin,cap_sys_nice+p) %{_bindir}/sol

# Systemd unit files for user services
%{_userunitdir}/*.service

# Udev rules
%{_udevrulesdir}/*-sol.rules

# Modules-load configuration
%{_modulesloaddir}/*-sol.conf

# Desktop entries
%{_datadir}/applications/*.desktop

# Icons
%{_datadir}/icons/hicolor/scalable/apps/*.Sol.svg
