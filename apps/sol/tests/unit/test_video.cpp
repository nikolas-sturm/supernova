...
struct AmfMaxAuSizeConfigTest: BaseTest, testing::WithParamInterface<AmfMaxAuSizeConfigParam> {
  void SetUp() override {
    ...
    config::stream.file_apps = SOL_SOURCE_DIR "/tests/unit/test_video.cpp";
  }
  ...
  config::sol_t original_sol {config::sol};  ///< Core configuration restored after each parameterized test.
