...
protected:
  ...
  void SetUp() override {
    ...
    // Save current config
    saved_username = config::sol.username;
    saved_password = config::sol.password;
    saved_salt = config::sol.salt;
    saved_locale = config::sol.locale;
    saved_csrf_allowed_origins = config::sol.csrf_allowed_origins;

    // Set up test credentials
    config::sol.username = "testuser";
    config::sol.salt = "testsalt";
    config::sol.password = util::hex(crypto::hash("testpass" + config::sol.salt)).to_string();

    // Set test locale
    config::sol.locale = "en";
    ...
    config::sol.csrf_allowed_origins = {
      "https://localhost",
      "https://127.0.0.1",
      "https://[::1]"
    };
    ...
    test_web_dir = std::filesystem::temp_directory_path() / "sol_test_confighttp";  // NOSONAR(cpp:S5443): safe for tests
    ...
    config::sol.csrf_allowed_origins.push_back(std::format("https://localhost:{}", port));
    config::sol.csrf_allowed_origins.push_back(std::format("https://127.0.0.1:{}", port));
    config::sol.csrf_allowed_origins.push_back(std::format("https://[::1]:{}", port));
  ...
  void TearDown() override {
    ...
    config::sol.username = saved_username;
    config::sol.password = saved_password;
    config::sol.salt = saved_salt;
    config::sol.locale = saved_locale;
    config::sol.csrf_allowed_origins = saved_csrf_allowed_origins;
    nvhttp::expire_pair_sessions(std::chrono::steady_clock::time_point::max());
...
protected:
  ...
  void SetUp() override {
    ...
    state_file = std::filesystem::temp_directory_path() / "sol_confighttp_client_update_state.json";
...
TEST_F(ConfigHttpTest, GetPageNoRedirectWhenUsernameEmpty) {
  ...
  const std::string saved = config::sol.username;
  config::sol.username = "";
...
protected:
  ...
  void SetUp() override {
    ...
    browse_test_dir = std::filesystem::temp_directory_path() / "sol_browse_test";  // NOSONAR(cpp:S5443): safe for tests
...
TEST_F(BrowseDirectoryTest, BrowseTrulyNonexistentPathReturnsBadRequest) {
  ...
  const std::string nonexistent = "/sol_nonexistent_xyz_54321/also_nonexistent";
...
TEST_F(BrowseDirectoryTest, BuildBrowseEntries_NonexistentDirectory_ReturnsEmptyArray) {
  const auto entries = confighttp::build_browse_entries("/sol_nonexistent_dir_xyz_99999", "any");
  ASSERT_TRUE(entries.is_array());
  ASSERT_TRUE(entries.empty());
}
