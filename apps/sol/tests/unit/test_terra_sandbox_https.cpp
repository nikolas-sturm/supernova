/**
 * @file tests/unit/test_terra_sandbox_https.cpp
 * @brief Direct HTTPS route validation for the Terra sandbox API.
 */
...
namespace {
   ...
   */
  class TerraSandboxHttpsTest: public BaseTest {
  public:
    /**
  ...
  protected:
    ...
    void SetUp() override {
      ...
      fixture_dir = fs::path {SOL_TEST_BIN_DIR} / "terra_sandbox_https";
      ...
      const auto host = test_utils::certificates::generate_ca_credentials("Sol HTTPS Test Host");
      ...
      const auto scoped = test_utils::certificates::generate_ca_credentials("Sol HTTPS Scoped Client");
      ...
      const auto unscoped = test_utils::certificates::generate_ca_credentials("Sol HTTPS Unscoped Client");
      unscoped_uuid = nvhttp::test_support::add_client("sandbox-unscoped", unscoped.x509, true);
      ASSERT_FALSE(unscoped_uuid.empty());
      terra_api::client_permissions_t restricted;
      ...
      const auto unknown = test_utils::certificates::generate_ca_credentials("Sol HTTPS Unknown Client");
  ...
  TEST_F(TerraSandboxHttpsTest, RejectsClientWithoutCertificateAtHandshake) {
  ...
  TEST_F(TerraSandboxHttpsTest, RejectsUnpairedClientCertificate) {
  ...
  TEST_F(TerraSandboxHttpsTest, RejectsMissingSandboxScope) {
  ...
  TEST_F(TerraSandboxHttpsTest, ListsEmptySandboxCollection) {
  ...
  TEST_F(TerraSandboxHttpsTest, RejectsMalformedSinceParameter) {
  ...
  TEST_F(TerraSandboxHttpsTest, RejectsUnknownSandboxResource) {
  ...
  TEST_F(TerraSandboxHttpsTest, CreateRequiresIdempotencyKey) {
  ...
  TEST_F(TerraSandboxHttpsTest, CreateRejectsUnknownProfileAndWorkspaceAssociation) {
  ...
  TEST_F(TerraSandboxHttpsTest, DeleteRequiresStrongRevisionPrecondition) {
  ...
  TEST_F(TerraSandboxHttpsTest, LifecycleRoundTripThroughDurableOperations) {
...
#else
TEST(TerraSandboxHttpsTest, WindowsOnly) {
  GTEST_SKIP() << "Terra sandbox routes are Windows-specific";
}
#endif
