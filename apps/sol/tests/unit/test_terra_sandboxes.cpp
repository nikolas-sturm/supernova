/**
 * @file tests/unit/test_terra_sandboxes.cpp
 * @brief Tests for standalone Terra sandbox lifecycle core.
 */
...
namespace {
  using namespace terra_sandboxes;
...
TEST(TerraSandboxesPolicyTest, NormalizesEverySecureDefaultExactly) {
...
TEST(TerraSandboxesPolicyTest, AcceptsCompletePolicyAndSecretReferences) {
...
TEST(TerraSandboxesPolicyTest, RejectsUnknownNestedMalformedAndOutOfRangeValues) {
...
TEST(TerraSandboxesTest, CreatesListsSerializesAndRollsBackFailedWrite) {
  fake_t fake;
...
TEST(TerraSandboxesTest, ValidatesCreationAndRejectsUnenforceablePolicy) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
...
TEST(TerraSandboxesTest, ResolvesBeforeCapabilityAndLaunchThenStops) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
...
TEST(TerraSandboxesTest, AppliesValidatedLaunchConfigurationOverride) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
  start_t request;
...
TEST(TerraSandboxesTest, FailsBeforeExecutionForResolutionOrCapabilityFailure) {
  fake_t unresolved;
...
TEST(TerraSandboxesTest, CleansFailedLaunchAndRollsBackSuccessfulLaunchOnWriteFailure) {
  fake_t failed;
...
TEST(TerraSandboxesTest, RejectsAndCleansDuplicateLaunchResourceIds) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
...
TEST(TerraSandboxesTest, RestartsThroughConfirmedTermination) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
...
TEST(TerraSandboxesTest, RevocationOrphansPersistentDeletesEphemeralAndAllowsAdoption) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
...
TEST(TerraSandboxesTest, DeletesOnlyAfterTerminationAndCleanup) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
  ASSERT_EQ(manager.start(SANDBOX, 1).status, status_t::success);
...
TEST(TerraSandboxesTest, CleanupFailureClearsDeadRuntimeAndBlocksRelaunch) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
  ASSERT_EQ(manager.start(SANDBOX, 1).status, status_t::success);
...
TEST(TerraSandboxesTest, DeleteSaveFailureRetainsDeletingIntentAndDisablesManager) {
  fake_t fake;
  int saves = 0;
...
TEST(TerraSandboxesTest, ReconcilesAbnormalExitAndPublishesEveryTransition) {
  fake_t fake;
...
TEST(TerraSandboxesTest, ReconcilePersistenceFailureDisablesManager) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
  ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
  ASSERT_EQ(manager.start(SANDBOX, 1).status, status_t::success);
...
TEST(TerraSandboxesTest, MalformedRecordFailsWholeDocumentClosed) {
  fake_t fake;
  {
    manager_t manager {fake.callbacks()};
    ASSERT_EQ(manager.create(OWNER, creation()).status, status_t::success);
...
TEST(TerraSandboxesTest, SerializesConcurrentCreation) {
  fake_t fake;
  manager_t manager {fake.callbacks()};
