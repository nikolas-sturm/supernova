 ...
 * runs on. Sol is responsible for the platform-independent half of the translation:
 * applying the `keybindings` remap, tracking which modifiers the client holds, and injecting
 * synthetic modifier presses when the client reports a modifier Sol does not believe is
...
namespace {
  ...
  /**
   * @brief Non-character keys a client can send that Sol must forward verbatim.
   */
  ...
  /**
   * @brief Every virtual-key code Sol recognizes as a modifier key.
   */
  ...
  constexpr std::array<modifier_t, 4> modifiers {{
    ...
    // Meta has no side-agnostic code and Sol never synthesizes it.
...
TEST_F(KeyboardPassthroughTest, CoversTheWholePrintableAsciiRange) {
  ...
  // A client types a shifted character by holding a real shift key, so Sol must forward
...
TEST_F(KeyboardPassthroughTest, ForwardsModifierKeysWithoutSyntheticInjection) {
  ...
  // claims modifiers Sol does not think are held.
...
TEST_F(KeyboardPassthroughTest, InjectsSyntheticModifiersTheClientHoldsWithoutSendingKeys) {
  ...
  // client's own compositor swallowed it. Sol wraps the key in a real modifier press.
  struct injectable_t {
    unsigned bit;
