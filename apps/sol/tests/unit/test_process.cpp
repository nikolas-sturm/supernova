...
protected:
  void SetUp() override {
    ...
    test_dir = fs::temp_directory_path() / "sol_process_png_test";  // NOSONAR(cpp:S5443): safe for tests
...
TEST_F(ProcessPNGTest, CheckValidPNG_RealFile) {
  // Test with the actual sol.png from the project root

  // Only run this test if the file exists
  if (const fs::path sol_png = fs::path(SOL_SOURCE_DIR) / "sol.png"; fs::exists(sol_png)) {
    EXPECT_TRUE(proc::check_valid_png(sol_png));
  } else {
    GTEST_SKIP() << "sol.png not found in project root";
  }
}
...
TEST_F(ProcessPNGTest, ValidateAppImagePath_OldSteamDefault) {
  ...
  EXPECT_EQ(result, SOL_ASSETS_DIR "/steam.png");
}

TEST_F(ProcessPNGTest, PersistsStableTerraIdentityAcrossMutableMetadataChanges) {
...
TEST_F(ProcessPNGTest, RepairsDuplicateTerraIdentitiesAndLegacyIds) {
...
TEST_F(ProcessPNGTest, RemovesMalformedOptionalTerraMetadata) {
  const auto catalog = test_dir / "malformed-terra-metadata.json";
  const nlohmann::json source {
    {"env", nlohmann::json::object()},
  ...
  const auto &metadata = parsed->get_apps().front().terra_metadata;
