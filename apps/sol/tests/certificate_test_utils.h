 ...
 * @brief Certificate fixtures shared by Sol security tests.
 */
#pragma once
...
namespace test_utils::certificates {
   ...
   */
  inline crypto::creds_t generate_ca_credentials(const std::string_view common_name = "Sol Test Client CA") {
  ...
  inline crypto::creds_t generate_derived_leaf(const crypto::creds_t &issuer_credentials) {
    auto credentials = crypto::gen_creds("Sol Test Derived Client", 2048);
    auto certificate = crypto::x509(credentials.x509);
