...
namespace http {
  ...
  int init() {
    ...
    if (clean_slate) {
      ...
      auto dir = std::filesystem::temp_directory_path() / "Sol"sv;
    ...
    if (!user_creds_exist(config::sol.credentials_file)) {
      BOOST_LOG(info) << "Open the Web UI to set your new username and password and getting started";
    } else if (reload_user_creds(config::sol.credentials_file)) {
      return -1;
    }
    return 0;
  }
  ...
  int reload_user_creds(const std::string &file) {
    ...
    try {
      pt::read_json(file, inputTree);
      config::sol.username = inputTree.get<std::string>("username");
      config::sol.password = inputTree.get<std::string>("password");
      config::sol.salt = inputTree.get<std::string>("salt");
    } catch (std::exception &e) {
  ...
  int create_creds(const std::string &pkey, const std::string &cert) {
    ...
    auto creds = crypto::gen_creds("Sol Gamestream Host"sv, 2048);
