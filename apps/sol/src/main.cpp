/**
 * @file src/main.cpp
 * @brief Definitions for the main entry point for Sol.
 */
// standard includes
#include <codecvt>
#include <csignal>
...
/**
 * @brief Forward a POSIX signal to the registered Sol handler.
 *
...
/**
 * @brief Run the main event loop until Sol is asked to exit.
 *
...
#ifndef _WIN32
  run_loop = tray_is_enabled && config::sol.system_tray;  // On Windows, tray runs in separate thread, so no main loop needed for tray
#endif
...
#pragma GCC diagnostic pop
  ...
  auto log_deinit_guard = logging::init(config::sol.min_log_level, config::sol.log_file);
  if (!log_deinit_guard) {
  ...
  if (!config::sol.cmd.name.empty()) {
    auto fn = cmd_to_func.find(config::sol.cmd.name);
    if (fn == std::end(cmd_to_func)) {
      BOOST_LOG(fatal) << "Unknown command: "sv << config::sol.cmd.name;
    ...
    return fn->second(argv[0], config::sol.cmd.argc, config::sol.cmd.argv);
  }
  ...
  if (nvprefs_instance.load()) {
    // Restore global settings to the undo file left by improper termination of sol.exe
    nvprefs_instance.restore_from_and_delete_undo_file_if_exists();
    // Modify application settings for sol.exe
  ...
  // Wait as long as possible to terminate Sol.exe during logoff/shutdown
  ...
  std::jthread session_monitor_thread([&]() {
    ...
    WNDCLASSA wnd_class {};
    wnd_class.lpszClassName = "SolSessionMonitorClass";
  ...
  on_signal(SIGINT, [&force_shutdown, &display_device_deinit_guard, shutdown_event]() {
    ...
    auto task = []() {
      BOOST_LOG(fatal) << "10 seconds passed, yet Sol's still running: Forcing shutdown"sv;
      logging::log_flush();
      lifetime::debug_trap();
    };
    ...
    if (tray_is_enabled && config::sol.system_tray) {
  ...
  // If any of the following fail, we log an error and continue event though sol will not function correctly.
  ...
  if (config::sol.port == 47989 && is_gamestream_enabled()) {
    BOOST_LOG(fatal) << "GameStream is still enabled in GeForce Experience! This *will* cause streaming problems with Sol!"sv;
    BOOST_LOG(fatal) << "Disable GameStream on the SHIELD tab in GeForce Experience or change the Port setting on the Advanced tab in the Sol Web UI."sv;
  }
#endif
  ...
  if (tray_is_enabled && config::sol.system_tray) {
    ...
    // the tray icon would not appear even though Sol is running correctly otherwise.
