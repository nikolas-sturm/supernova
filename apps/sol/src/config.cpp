/**
 * @file src/config.cpp
 * @brief Definitions for the configuration of Sol.
 */
// standard includes
#include <algorithm>
#include <filesystem>
...
constexpr auto CA_DIR = "credentials";  ///< Subdirectory under app data that stores Sol credentials.
...
#undef _CONVERT_2_ARG_
  ...
  /**
   * @brief Default top-level Sol configuration values used before file and CLI overrides.
   */
  sol_t sol {
    "en",  // locale
    2,  // min_log_level
    ...
    "ipv4",  // Address family
    {},  // Bind address
    platf::appdata().string() + "/sol.log",  // log file
  ...
  /**
   * @brief Parse Sol configuration text into key-value entries.
   */
  ...
  void list_int_f(std::unordered_map<std::string, std::string> &vars, const std::string &name, std::vector<int> &input) {
    ...
    // If the list is not cleared, then the specified parameters do not affect the behavior of the sol server.
  ...
  /**
   * @brief Apply single-character command-line flags to the global Sol flags bitset.
   *
  ...
  int apply_flags(const char *line) {
    ...
    while (*line != '\0') {
      switch (*line) {
        case '0':
          config::sol.flags[config::flag::PIN_STDIN].flip();
          break;
        case '1':
          config::sol.flags[config::flag::FRESH_STATE].flip();
          break;
        case '2':
          config::sol.flags[config::flag::FORCE_VIDEO_HEADER_REPLACE].flip();
          break;
        case 'p':
          config::sol.flags[config::flag::UPNP].flip();
          break;
        default:
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    bool_f(vars, "nvenc_latency_over_power", video.nv_sol_high_power_mode);
    ...
    string_f(vars, "sunshine_name", nvhttp.sol_name);
    path_f(vars, "log_path", config::sol.log_file);
    path_f(vars, "file_state", nvhttp.file_state);

    // Must be run after "file_state"
    config::sol.credentials_file = config::nvhttp.file_state;
    path_f(vars, "credentials_file", config::sol.credentials_file);

    string_f(vars, "external_ip", nvhttp.external_ip);
    list_prep_cmd_f(vars, "global_prep_cmd", config::sol.prep_cmds);
    ...
    for (const auto &origin : user_csrf_origins) {
      if (origin.size() > 8 && origin.starts_with("https://")) {
        sol.csrf_allowed_origins.push_back(origin);
      } else {
    ...
    if (!fs::exists(stream.file_apps.c_str())) {
      fs::copy_file(SOL_ASSETS_DIR "/apps.json", stream.file_apps);
      fs::permissions(
        stream.file_apps,
    ...
    bool_f(vars, "notify_pre_releases", sol.notify_pre_releases);
    bool_f(vars, "system_tray", sol.system_tray);

    int port = sol.port;
    int_between_f(vars, "port"s, port, {1024 + nvhttp::PORT_HTTPS, 65535 - rtsp_stream::RTSP_SETUP_PORT});
    sol.port = (std::uint16_t) port;
    ...
    const unsigned short web_ui_port = sol.port + 1;
    sol.csrf_allowed_origins.push_back(std::format("https://localhost:{}", web_ui_port));
    sol.csrf_allowed_origins.push_back(std::format("https://127.0.0.1:{}", web_ui_port));
    sol.csrf_allowed_origins.push_back(std::format("https://[::1]:{}", web_ui_port));

    string_restricted_f(vars, "address_family", sol.address_family, {"ipv4"sv, "both"sv});
    string_f(vars, "bind_address", sol.bind_address);
    ...
    string_restricted_f(vars, "locale", config::sol.locale, {
                                                                   "bg"sv,  // Bulgarian
                                                                   "cs"sv,  // Czech
                                                                   "de"sv,  // German
                                                                   "en"sv,  // English
  ...
  int parse(int argc, char *argv[]) {
    ...
    for (auto x = 1; x < argc; ++x) {
      ...
      if (line == "--help"sv) {
        ...
        if (*(line + 1) == '-') {
          sol.cmd.name = line + 2;
          sol.cmd.argc = argc - x - 1;
          sol.cmd.argv = argv + x + 1;
    ...
    try {
      ...
      if (!fs::exists(sol.config_file)) {
        std::ofstream {sol.config_file};
      }

      // Read config file
      auto vars = parse_config(file_handler::read_file(sol.config_file.c_str()));
    ...
    if (!config_loaded && !shortcut_launch) {
      BOOST_LOG(fatal) << "To relaunch Sol successfully, use the shortcut in the Start Menu. Do not run Sol.exe manually."sv;
    ...
    if (service_admin_launch) {
      ...
      // Always return 1 to ensure Sol doesn't start normally
      return 1;
    }
    if (shortcut_launch) {
