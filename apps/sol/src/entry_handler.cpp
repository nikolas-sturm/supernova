...
namespace args {
  int creds(const char *name, int argc, char *argv[]) {
    ...
    http::save_user_creds(config::sol.credentials_file, argv[0], argv[1]);
...
namespace lifetime {
  ...
  void exit_sol(int exit_code, bool async) {
    // Store the exit code of the first exit_sol() call
    int zero = 0;
...
void log_publisher_data() {
  BOOST_LOG(info) << "Package Publisher: "sv << SOL_PUBLISHER_NAME;
  BOOST_LOG(info) << "Publisher Website: "sv << SOL_PUBLISHER_WEBSITE;
  BOOST_LOG(info) << "Get support: "sv << SOL_PUBLISHER_ISSUE_URL;
}
...
namespace service_ctrl {
  /**
   * @brief Owns Windows service-manager handles for the Sol service.
   */
  class service_controller {
  public:
    /**
     * @brief Open the Windows service manager and Sol service handle.
     *
    ...
    service_controller(DWORD service_desired_access) {
      ...
      service_handle = OpenServiceA(scm_handle, "SolService", service_desired_access);
      if (!service_handle) {
        auto winerr = GetLastError();
    ...
    /**
     * @brief Asynchronously starts the Sol service.
     *
