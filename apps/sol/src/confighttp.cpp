...
namespace confighttp {
  ...
  /**
   * @brief HTTPS server type used for Sol's configuration UI.
   */
  ...
  void send_unauthorized(const resp_https_t &response, const req_https_t &request) {
    ...
    const SimpleWeb::CaseInsensitiveMultimap headers {
      ...
      {"WWW-Authenticate", R"(Basic realm="Sol Gamestream Host", charset="UTF-8")"},
  ...
  bool authenticate(const resp_https_t &response, const req_https_t &request) {
    ...
    if (const auto hash = util::hex(crypto::hash(password + config::sol.salt)).to_string(); !boost::iequals(username, config::sol.username) || hash != config::sol.password) {
      return false;
    }
  ...
  std::string get_client_id(const req_https_t &request) {
    ...
    if (const auto auth = request->header.find("authorization"); !config::sol.username.empty() && auth != request->header.end()) {
  ...
  bool validate_csrf_token(const resp_https_t &response, const req_https_t &request, const std::string &client_id) {
    ...
    auto is_allowed_origin = [](const std::string_view url) {
      return std::ranges::any_of(config::sol.csrf_allowed_origins, [&url](const std::string &allowed_origin) {
    ...
    if (header_it == request->header.end()) {
      ...
      if (query_it == query_params.end()) {
        ...
        BOOST_LOG(error) << "Web UI: To allow this origin, add it to the 'csrf_allowed_origins' option in your Sol configuration"sv;
  ...
  void getPage(const resp_https_t &response, const req_https_t &request, const char *html_file, const bool require_auth, const bool redirect_if_username) {
    ...
    if (redirect_if_username && !config::sol.username.empty()) {
   ...
   * @param response The HTTP response object.
   * @param request The HTTP request object.
   * @todo combine function with getSolLogoImage and possibly getNodeModules
   * @todo use mime_types map
   */
  void getFaviconImage(const resp_https_t &response, const req_https_t &request) {
    print_req(request);

    std::ifstream in(WEB_DIR "images/sol.ico", std::ios::binary);
    SimpleWeb::CaseInsensitiveMultimap headers;
  ...
  /**
   * @brief Get the Sol logo image.
   * @param response The HTTP response object.
   * @param request The HTTP request object.
   * @todo combine function with getFaviconImage and possibly getNodeModules
   * @todo use mime_types map
   */
  void getSolLogoImage(const resp_https_t &response, const req_https_t &request) {
    print_req(request);

    std::ifstream in(WEB_DIR "images/logo-sol-45.png", std::ios::binary);
    SimpleWeb::CaseInsensitiveMultimap headers;
  ...
  void getApps(const resp_https_t &response, const req_https_t &request) {
    ...
    try {
      ...
      // Legacy versions of Sol used strings for boolean and integers, let's convert them
      ...
      if (input_tree.contains("scopes") || input_tree.contains("allowed_apps") || input_tree.contains("input") || input_tree.contains("expires_at")) {
        ...
        if (input_tree.contains("scopes")) {
          ...
          for (const auto &scope_value : input_tree.at("scopes")) {
            ...
            if (!terra_api::is_known_scope(scope)) {
              throw std::invalid_argument(std::format("unknown Terra scope: {}", scope));
            }
  ...
  void getConfig(const resp_https_t &response, const req_https_t &request) {
    ...
    nlohmann::json output_tree;
    output_tree["status"] = true;
    output_tree["platform"] = SOL_PLATFORM;
    output_tree["version"] = PROJECT_VERSION;

    auto vars = config::parse_config(file_handler::read_file(config::sol.config_file.c_str()));
  ...
  void getLocale(const resp_https_t &response, const req_https_t &request) {
    ...
    nlohmann::json output_tree;
    output_tree["status"] = true;
    output_tree["locale"] = config::sol.locale;
    send_response(response, output_tree);
  }
  ...
  void saveConfig(const resp_https_t &response, const req_https_t &request) {
    ...
    try {
      ...
      file_handler::write_file(config::sol.config_file.c_str(), config_stream.str());
      output_tree["status"] = true;
      send_response(response, output_tree);
    } catch (std::exception &e) {
  ...
  void getLogs(const resp_https_t &response, const req_https_t &request) {
    ...
    std::string content = file_handler::read_file(config::sol.log_file.c_str());
    SimpleWeb::CaseInsensitiveMultimap headers;
  ...
  void savePassword(const resp_https_t &response, const req_https_t &request) {
    ...
    if (!config::sol.username.empty() && !authenticate(response, request)) {
      return;
    }
    ...
    try {
      ...
      if (newUsername.empty()) {
        ...
        auto hash = util::hex(crypto::hash(password + config::sol.salt)).to_string();
        if (config::sol.username.empty() || (boost::iequals(username, config::sol.username) && hash == config::sol.password)) {
          if (newPassword.empty() || newPassword != confirmPassword) {
            errors.emplace_back("Password Mismatch");
          } else {
            http::save_user_creds(config::sol.credentials_file, newUsername, newPassword);
            http::reload_user_creds(config::sol.credentials_file);
            output_tree["status"] = true;
          }
        } else {
  ...
  /**
   * @brief Authenticate a Web UI request and restart the Sol process.
   *
   ...
   * never logged or saved in Sol's configuration, and extracted mutable copies
...
#if defined(_WIN32) && defined(SOL_TRAY) && SOL_TRAY >= 1
  ...
  void start() {
    ...
    const auto address_family = net::af_from_enum_string(config::sol.address_family);
    ...
    server.resource["^/images/sol.ico$"]["GET"] = getFaviconImage;
    server.resource["^/images/logo-sol-45.png$"]["GET"] = getSolLogoImage;
    ...
    const auto display_addr = config::sol.bind_address.empty() ? "localhost"sv : std::string_view {bind_addr};
