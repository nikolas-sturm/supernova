...
namespace platf {
  ...
  bool enabled_mouse_keys = false;  ///< Tracks whether Windows Mouse Keys was enabled before Sol changed it.
  ...
  std::filesystem::path appdata() {
    WCHAR sol_path[MAX_PATH];
    GetModuleFileNameW(nullptr, sol_path, _countof(sol_path));
    return std::filesystem::path {sol_path}.remove_filename() / L"config"sv;
  }
  ...
  HANDLE retrieve_users_token(bool elevated) {
    ...
    if (elevated && (elevationType == TokenElevationTypeDefault && !IsUserAdmin(userToken))) {
      ...
      BOOST_LOG(warning) << "This command requires elevation and the current user account logged in does not have administrator rights. "
                         << "For security reasons Sol will retain the same access level as the current user and will not elevate it.";
    }
  ...
  STARTUPINFOEXW create_startup_info(FILE *file, HANDLE *job, std::error_code &ec) {
    ...
    if (file) {
      ...
      // our inheritable handles, such as our own log file handle created by SolSvc).
      //
  ...
  bp::child run_command(bool elevated, bool interactive, const std::string &cmd, boost::filesystem::path &working_dir, const bp::environment &env, FILE *file, std::error_code &ec, bp::group *group) {
    ...
    sol_wenv[path_var_name].assign(start_dir + L";" + old_path_val);

    // Restore the old PATH value for our process when we're done here
    auto restore_path = util::fail_guard([&]() {
      if (old_path_val.empty()) {
        sol_wenv[path_var_name].clear();
      } else {
        sol_wenv[path_var_name].assign(old_path_val);
      }
    });
    ...
    if (is_running_as_system()) {
      ...
      if (!user_token) {
        // Fail the launch rather than risking launching with Sol's permissions unmodified.
        ec = std::make_error_code(std::errc::permission_denied);
        return bp::child();
      }
    ...
    // This will inherit the elevation of whatever the user launched Sol with.
    else {
  ...
  void streaming_will_start() {
    ...
    // Modify NVIDIA control panel settings again, in case they have been changed externally since sol launch
  ...
  void restart_on_exit() {
    ...
    if (GetModuleFileNameW(nullptr, executable, ARRAYSIZE(executable)) == 0) {
      auto winerr = GetLastError();
      BOOST_LOG(fatal) << "Failed to get Sol path: "sv << winerr;
      return;
    }
    ...
    if (!CreateProcessW(executable, GetCommandLineW(), nullptr, nullptr, false, CREATE_UNICODE_ENVIRONMENT | EXTENDED_STARTUPINFO_PRESENT, nullptr, nullptr, (LPSTARTUPINFOW) &startup_info, &process_info)) {
      auto winerr = GetLastError();
      BOOST_LOG(fatal) << "Unable to restart Sol: "sv << winerr;
      return;
    }
