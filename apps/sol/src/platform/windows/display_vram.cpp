...
#if !defined(SOL_SHADERS_DIR)  // for testing this needs to be defined in cmake as we don't do an install
  /**
   * @def SOL_SHADERS_DIR
   * @brief Macro for SOL SHADERS DIR.
   */
  #define SOL_SHADERS_DIR SOL_ASSETS_DIR "/shaders/directx"
#endif
namespace platf {
...
namespace platf::dxgi {
  ...
  public:
     ...
     */
    void apply_colorspace(const ::video::sol_colorspace_t &colorspace) {
...
#undef create_pixel_shader_helper
  ...
  public:
     ...
     */
    bool init_encoder(const ::video::config_t &client_config, const ::video::sol_colorspace_t &colorspace) override {
      if (!nvenc_d3d) {
        return false;
      }
    ...
    if (capture_format == DXGI_FORMAT_UNKNOWN) {
      ...
      if (src) {
        ...
        if (blend_mouse_cursor_flag) {
          ...
          // or there's mouse cursor on screen, but its drawing is disabled in sol.
  ...
  #define compile_vertex_shader_helper(x) \
    if (!(x##_hlsl = compile_vertex_shader(SOL_SHADERS_DIR "/" #x ".hlsl"))) \
      return -1;
#endif
#ifndef DOXYGEN
  #define compile_pixel_shader_helper(x) \
    if (!(x##_hlsl = compile_pixel_shader(SOL_SHADERS_DIR "/" #x ".hlsl"))) \
      return -1;
#endif
