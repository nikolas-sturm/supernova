...
namespace {
  ...
  const auto sol_program_data_folder = "Sunshine";
...
namespace nvprefs {
  ...
  bool nvprefs_interface::load() {
    if (!pimpl->loaded) {
      ...
      pimpl->undo_folder_path = std::filesystem::path(program_data_env) / sol_program_data_folder;
  ...
  bool nvprefs_interface::modify_application_profile() {
    ...
    // Modify and save sol.exe application profile settings, if needed
    bool modified = false;
  ...
  bool nvprefs_interface::modify_global_profile() {
    ...
    auto make_undo_and_commit = [&]() -> bool {
      ...
      if (!pimpl->undo_file) {
        // Prepare Sol folder in ProgramData if it doesn't exist
