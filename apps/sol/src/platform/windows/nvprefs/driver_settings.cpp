...
namespace {
  ...
  const auto sol_application_profile_name = L"SolStream";
  const auto sol_application_path = L"sol.exe";
...
namespace nvprefs {
  ...
  bool driver_settings_t::check_and_modify_application_profile(bool &modified) {
    ...
    fill_nvapi_string(profile_name, sol_application_profile_name);
    ...
    if (status != NVAPI_OK) {
      ...
      fill_nvapi_string(profile.profileName, sol_application_profile_name);
    ...
    NvAPI_UnicodeString sol_path = {};
    fill_nvapi_string(sol_path, sol_application_path);

    NVDRS_APPLICATION application = {};
    application.version = NVDRS_APPLICATION_VER_V1;
    status = NvAPI_DRS_GetApplicationInfo(session_handle, profile_handle, sol_path, &application);

    if (status != NVAPI_OK) {
      // Add application to application profile if missing
      application.version = NVDRS_APPLICATION_VER_V1;
      application.isPredefined = 0;
      fill_nvapi_string(application.appName, sol_application_path);
      fill_nvapi_string(application.userFriendlyName, sol_application_path);
    ...
    if (!get_nvprefs_options().sol_high_power_mode) {
      if (status == NVAPI_OK && setting.settingLocation == NVDRS_CURRENT_PROFILE_LOCATION) {
        // User requested to not use high power mode for sol.exe,
        ...
        info_message(std::wstring(L"Removed PREFERRED_PSTATE for ") + sol_application_path);
      }
      ...
      info_message(std::wstring(L"Changed PREFERRED_PSTATE to PREFERRED_PSTATE_PREFER_MAX for ") + sol_application_path);
    }
