...
namespace platf {
  ...
  /**
   * @brief Feedback command sent from Sol to a virtual gamepad.
   */
  ...
  /**
   * @brief Convert a Sol pixel format enum to its string name.
   *
...
#undef _CONVERT
  ...
  struct encode_device_t {
    ...
    video::sol_colorspace_t colorspace;  ///< Colorspace metadata expected by the encoder.
  };
  ...
  struct nvenc_encode_device_t: encode_device_t {
     ...
     */
    virtual bool init_encoder(const video::config_t &client_config, const video::sol_colorspace_t &colorspace) = 0;
  ...
  constexpr auto SERVICE_NAME = "Sol";  ///< mDNS service instance name advertised for GameStream discovery.
