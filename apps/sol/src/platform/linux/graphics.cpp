...
#define DRM_FORMAT_MOD_INVALID fourcc_mod_code(0, ((1ULL << 56) - 1))
   ...
   */
  #define SOL_SHADERS_DIR SOL_ASSETS_DIR "/shaders/opengl"
#endif
...
namespace egl {
  ...
  void sws_t::apply_colorspace(const video::sol_colorspace_t &colorspace, bool is_yuv444) {
  ...
  std::optional<sws_t> sws_t::make_nv12(int in_width, int in_height, int out_width, int out_height, gl::tex_t &&tex) {
...

    {
      constexpr std::array<const char *, 5> sources {{
        SOL_SHADERS_DIR "/ConvertUV.frag",
        SOL_SHADERS_DIR "/ConvertUV.vert",
        SOL_SHADERS_DIR "/ConvertY.frag",
        SOL_SHADERS_DIR "/Scene.vert",
        SOL_SHADERS_DIR "/Scene.frag",
      }};
  ...
  std::optional<sws_t> sws_t::make_yuv444(int in_width, int in_height, int out_width, int out_height, gl::tex_t &&tex) {
...

    {
      constexpr std::array<const char *, 5> sources {{
        SOL_SHADERS_DIR "/Scene.vert",
        SOL_SHADERS_DIR "/ConvertV.frag",
        SOL_SHADERS_DIR "/ConvertU.frag",
        SOL_SHADERS_DIR "/ConvertY.frag",
        SOL_SHADERS_DIR "/Scene.frag",
      }};
