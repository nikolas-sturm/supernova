...
const bool SOL_USE_PIPEWIRE_OBJECT_SERIAL = pw_check_library_version(0, 3, 64);
...
constexpr bool SOL_USE_PIPEWIRE_OBJECT_SERIAL = true;
#else
...
constexpr bool SOL_USE_PIPEWIRE_OBJECT_SERIAL = false;  ///< Whether PipeWire object serials should be used for matching.
  /**
...
namespace pipewire {
  /**
   * @brief PipeWire SPA format mapped to Sol pixel format.
   */
  struct format_map_t {
  ...
  public:
    ...
    int ensure_stream(const platf::mem_type_e mem_type, const uint32_t width, const uint32_t height, const AVRational target_framerate, const struct dmabuf_format_info_t *dmabuf_infos, const int n_dmabuf_infos, const bool display_is_nvidia) {
      ...
      if (!stream_data.stream) {
        ...
        stream_data.stream = pw_stream_new(core, "Sol Video Capture", props);
        ...
        if (SOL_USE_PIPEWIRE_OBJECT_SERIAL && (object_serial & SPA_ID_INVALID) != SPA_ID_INVALID) {
    ...
    /**
     * @brief Copy PipeWire metadata into the Sol image descriptor.
     *
    ...
    /**
     * @brief Populate a Sol image descriptor from PipeWire DMA-BUF planes.
     *
    ...
    /**
     * @brief Copy the latest PipeWire frame into Sol's image buffer.
     *
