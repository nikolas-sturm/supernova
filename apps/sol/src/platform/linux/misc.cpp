...
#ifdef __GNUC__
  #define SOL_GNUC_EXTENSION __extension__
#else
  /**
   * @def SOL_GNUC_EXTENSION
   * @brief Macro for SOL GNUC EXTENSION.
   */
  #define SOL_GNUC_EXTENSION
#endif
...
namespace dyn {
  ...
  int load(void *handle, const std::vector<std::tuple<apiproc *, const char *>> &funcs, bool strict) {
    ...
    for (auto &func : funcs) {
      ...
      *fn = SOL_GNUC_EXTENSION(apiproc) dlsym(handle, name);
...
namespace platf {
...
#else
  #ifndef __FreeBSD__
    BOOST_LOG(info) << "Sol compiled without DRM support. Cannot control Linux DRM master state for "sv << path.string();
  #endif
  ...
  fs::path appdata() {
    ...
    std::call_once(migration_flag, []() {
      ...
      if (std::string migrate_envvar; migrate_config && found && lizardbyte::common::get_env("SOL_MIGRATE_CONFIG", migrate_envvar) && migrate_envvar == "1") {
        std::error_code ec;
  ...
  /**
   * @brief Request a Sol process restart on exit.
   */
  ...
  /**
   * @brief Restart the Sol process through the platform launcher.
   */
  void restart() {
  ...
  namespace source {
    ...
    enum source_e : std::size_t {
#ifdef SOL_BUILD_CUDA
      NVFBC,  ///< NvFBC
#endif
#ifdef SOL_BUILD_WAYLAND
      WAYLAND,  ///< Wayland
#endif
#ifdef SOL_BUILD_DRM
      KMS,  ///< KMS
      ...
      X11,  ///< X11
#endif
#ifdef SOL_BUILD_KWIN
      KWIN,  ///< KWin ScreenCast
#endif
#ifdef SOL_BUILD_PORTAL
      PORTAL,  ///< XDG PORTAL
#endif
  ...
  std::unique_ptr<deinit_t> init() {
...
#endif
#if defined(SOL_BUILD_X11) || defined(SOL_BUILD_CUDA)
    if (std::string v; lizardbyte::common::get_env("DISPLAY", v) && window_system != window_system_e::WAYLAND) {
      if (lizardbyte::common::get_env("WAYLAND_DISPLAY", v)) {
        BOOST_LOG(warning) << "Wayland detected, yet sol will use X11 for screencasting, screencasting will only work on XWayland applications"sv;
      }
