 ...
 *
 * Bypasses xdg-desktop-portal entirely. Sol connects directly to KWin's
 * Wayland protocol to obtain a PipeWire node_id, then streams frames via PipeWire.
 *
 * Chain: KWin -> Wayland kde_screencast -> PipeWire -> Sol
 */
// standard includes
#include <algorithm>
#include <array>
...
namespace kwin {
  ...
  public:
    ...
    static void setup() {
      ...
      // System: Check system XDG applications for permission (usually installed with Sol)
      ...
      for (const auto &path : std::filesystem::directory_iterator(user_applications)) {
        ...
        if (entry.starts_with(user_filepathprefix)) {
          ...
          if (!entry_executablepath.empty() && std::filesystem::exists(entry_executablepath)) {
            // This entry is for another sol executable that still exists
      ...
      if (create_file) {
        ...
        if (filestream.is_open()) {
          filestream << "[Desktop Entry]" << std::endl
                     ...
                     << "Comment=Sol KWin screencast permission" << std::endl
  ...
  private:
    ...
    static std::filesystem::path get_home_dir() {
      ...
      // Note: This should be thread-safe as we're always accessing the same entry for Sol
      ...
      if (kde_screencast_v1_) {
        ...
        if (screencast_permission_helper_t::is_newly_initialized()) {
          BOOST_LOG(error) << "[kwingrab] zkde_screencast_unstable_v1 not found in registry. "sv
                              ...
                              "Try restarting sol or set KWIN_WAYLAND_NO_PERMISSION_CHECKS=1 to fully disable permission checks."sv;
        } else {
          BOOST_LOG(error) << "[kwingrab] zkde_screencast_unstable_v1 not found in registry. Check permission desktop file "sv
                              "for sol binary or set KWIN_WAYLAND_NO_PERMISSION_CHECKS=1 to fully disable permission checks."sv;
        }
        return -1;
      }
...
namespace platf {
  ...
  std::vector<std::string> kwin_display_names() {
    if (has_elevated_privileges(false)) {
      // We're still in the probing phase of Sol startup. Dropping portal security early will break KMS.
