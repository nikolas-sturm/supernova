...
namespace platf {
  ...
  struct mic_attr_t: public mic_t {
    ...
    /**
     * @brief Deliver a captured audio sample to Sol's audio pipeline.
     *
  ...
  std::unique_ptr<mic_t> microphone(const std::uint8_t *mapping, int channels, std::uint32_t sample_rate, std::uint32_t frame_size, std::string source_name) {
    ...
    mic->mic.reset(
      pa_simple_new(nullptr, "sol", pa_stream_direction_t::PA_STREAM_RECORD, source_name.c_str(), "sol-record", &ss, &pa_map, &pa_attr, &status)
    );
  ...
  namespace pa {
    ...
    /**
     * @brief Forward a PulseAudio integer callback value into a Sol alarm.
     *
    ...
    /**
     * @brief PulseAudio server controller that creates and removes Sol sinks.
     */
    ...
    public:
      ...
      } index;  ///< PulseAudio module indexes for Sol-created null sinks.
      ...
      /**
       * @brief Initialize PulseAudio mainloop, context, and Sol null sinks.
       *
      ...
      int init() {
        ...
        ctx.reset(pa_context_new(pa_mainloop_get_api(loop.get()), "sol"));
      ...
      /**
       * @brief Unload a Sol-created PulseAudio null sink.
       *
       * @param i PulseAudio introspection info supplied to the callback.
      ...
      /**
       * @brief Query host and virtual sink names available to Sol.
       *
      ...
      std::optional<sink_t> sink_info() override {
        ...
        cb_t<pa_sink_info *> f = [&](ctx_t::pointer ctx, const pa_sink_info *sink_info, int eol) {
          ...
          // Ensure Sol won't create a sink that already exists.
