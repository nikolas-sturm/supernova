...
namespace net {
  ...
  /**
   * @brief Convert a Boost address family to Sol network enum value.
   */
  ...
  std::string get_bind_address(const af_e af) {
    ...
    if (!config::sol.bind_address.empty()) {
      return config::sol.bind_address;
    }
  ...
  std::uint16_t map_port(int port) {
    ...
    auto mapped_port = (std::uint16_t) ((int) config::sol.port + port);
   ...
   * @return Hostname-based instance name or "Sol" if hostname is invalid.
   */
  std::string mdns_instance_name(const std::string_view &hostname) {
    ...
    return !instancename.empty() ? instancename : "Sol";
  }
}  // namespace net
