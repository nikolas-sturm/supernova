...
namespace upnp {
  ...
  public:
    deinit_t() {
      ...
      mappings.assign({
        {{rtsp, rtsp, "TCP"s}, "Sol - RTSP"s},
        {{video, video, "UDP"s}, "Sol - Video"s},
        {{audio, audio, "UDP"s}, "Sol - Audio"s},
        {{control, control, "UDP"s}, "Sol - Control"s},
        {{gs_http, gs_http, "TCP"s}, "Sol - Client HTTP"s},
        {{gs_https, gs_https, "TCP"s}, "Sol - Client HTTPS"s},
      });
      ...
      if (net::from_enum_string(config::nvhttp.origin_web_ui_allowed) > net::LAN) {
        mappings.emplace_back(mapping_t {{wm_http, wm_http, "TCP"s}, "Sol - Web UI"s});
      }
    ...
    std::vector<mapping_t> mappings;  ///< Port mappings Sol should keep registered with the gateway.
  ...
  std::unique_ptr<platf::deinit_t> start() {
    if (!config::sol.flags[config::flag::UPNP]) {
      return nullptr;
    }
