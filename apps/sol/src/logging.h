...
/**
 * @brief Boost.Log asynchronous text sink used by Sol logging.
 */
...
namespace logging {
  ...
  /**
   * @brief Format a Boost.Log record for Sol log output.
   *
  ...
  public:
    ...
    min_max_avg_periodic_logger(boost::log::sources::severity_logger<int> &severity, std::string_view message, std::string_view units, std::chrono::seconds interval_in_seconds = std::chrono::seconds(20)):
        ...
        enabled(config::sol.min_log_level <= severity.default_severity()) {
    }
