...
namespace video {
  ...
  void avcodec_software_encode_device_t::apply_colorspace() {
    auto avcodec_colorspace = avcodec_colorspace_from_sol_colorspace(colorspace);
  ...
  encoder_t amdvce {
    ...
    ),
    {
      // Common options
      {
        ...
        {"skip_frame"s, 0},
        {"log_to_dbg"s, []() {
           return config::sol.min_log_level < 2 ? 1 : 0;
         }},
  ...
  void encode_run(
    ...
    // streaming to continue without requiring a full restart of Sol.
  ...
  bool validate_encoder(encoder_t &encoder, bool expect_failure) {
    ...
    encoder.h264[encoder_t::VUI_PARAMETERS] = encoder.h264[encoder_t::VUI_PARAMETERS] && !config::sol.flags[config::flag::FORCE_VIDEO_HEADER_REPLACE];
    encoder.hevc[encoder_t::VUI_PARAMETERS] = encoder.hevc[encoder_t::VUI_PARAMETERS] && !config::sol.flags[config::flag::FORCE_VIDEO_HEADER_REPLACE];
...
namespace video {
  /**
   * @brief Create an FFmpeg D3D11VA hardware device from Sol's DXGI device.
   */
