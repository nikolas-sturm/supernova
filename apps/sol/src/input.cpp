...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
  ...
  struct gamepad_t {
    ...
    // Afterwards, the gamepad state on sol won't match the state on Moonlight.
    // To prevent Sol from sending erroneous input data to the active application,
    // Sol forces the button to be in a specific state until the gamepad state matches that of
    // Moonlight once more.
  ...
  void passthrough(std::shared_ptr<input_t> &input, PNV_ABS_MOUSE_MOVE_PACKET packet) {
    ...
    if (!packet->width || !packet->height) {
      BOOST_LOG(warning) << "Terra passed invalid dimensions"sv;
  ...
  void passthrough(std::shared_ptr<input_t> &input, PNV_MOUSE_BUTTON_PACKET packet) {
     ...
     * As a result, Sol will left-click on hyperlinks in the browser before right-clicking
     *
     * This can be solved by delaying BUTTON_LEFT, however, any delay on input is undesirable during gaming
     * As a compromise, Sol will only put delays on BUTTON_LEFT when
  ...
  /**
   * @brief Release every pressed mouse button tracked by Sol.
   */
  /**
