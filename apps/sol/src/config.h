/**
 * @file src/config.h
 * @brief Declarations for the configuration of Sol.
 */
#pragma once
...
namespace config {
  ...
  struct video_t {
    ...
    bool nv_sol_high_power_mode;  ///< Request NVIDIA high-power mode for Sol.
    ...
    struct dd_t {
      ...
      /**
       * @brief Selects how Sol prepares the active display before streaming.
       */
      ...
      /**
       * @brief Selects how Sol chooses the stream display resolution.
       */
      ...
      /**
       * @brief Selects how Sol chooses the stream display refresh rate.
       */
      ...
      /**
       * @brief Selects how Sol handles HDR state for the stream display.
       */
  ...
  struct nvhttp_t {
    ...
    std::string sol_name;  ///< Host name advertised to Moonlight clients.

    std::string file_state;  ///< Path to the persisted Sol state file.
  ...
  /**
   * @brief Top-level Sol configuration and credential state.
   */
  struct sol_t {
    std::string locale;  ///< Locale selected for Sol UI and log messages.
    ...
    std::string config_file;  ///< Path to the active Sol configuration file.
    ...
    std::uint16_t port;  ///< TCP port used by Sol services.
    std::string address_family;  ///< Address family requested for listening sockets.
    std::string bind_address;  ///< Local address Sol should bind to.
