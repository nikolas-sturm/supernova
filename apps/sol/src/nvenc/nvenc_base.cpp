...
namespace {
  ...
  /**
   * @brief Get the display name for a Sol video format.
   *
   * @param video_format Sol video format identifier.
...
namespace NVENC_NAMESPACE {
  ...
  bool nvenc_base::create_encoder(
    const ::nvenc::nvenc_config &config,
    const video::config_t &client_config,
    const video::sol_colorspace_t &sol_colorspace,
    platf::pix_fmt_e sol_buffer_format
  ) {
    ...
    const auto colorspace = nvenc_colorspace_from_sol_colorspace(sol_colorspace);
    const auto buffer_format = nvenc_format_from_sol_format(sol_buffer_format);
