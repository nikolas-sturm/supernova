...
namespace proc {
  ...
  int proc_t::execute(int app_id, std::shared_ptr<rtsp_stream::launch_session_t> launch_session) {
    ...
    for (; _app_prep_it != std::end(_app.prep_cmds); ++_app_prep_it) {
      ...
      if (ec) {
        ...
        // This is to prevent the issue where users reboot their PC and need to log in with Sol.
  ...
  std::string validate_app_image_path(std::string app_image_path) {
    ...
    if (auto full_image_path = std::filesystem::path(SOL_ASSETS_DIR) / app_image_path; std::filesystem::exists(full_image_path)) {
    ...
    if (app_image_path == "./assets/steam.png") {
      ...
      return SOL_ASSETS_DIR "/steam.png";
    }
  ...
  /**
   * @brief Remove malformed optional Terra metadata before publishing catalog records.
   *
   ...
   */
  bool normalize_terra_metadata(nlohmann::json &metadata) {
    bool changed = false;
  ...
  std::optional<proc::proc_t> parse(const std::string &file_name) {
    ...
    try {
      ...
      for (auto &[_, app_node] : apps_node) {
        ...
        auto &terra = source_app["x-eclipse"];
        if (!terra.is_object()) {
          terra = nlohmann::json::object();
          migrate_catalog = true;
        }

        auto uuid = terra.contains("uuid") && terra["uuid"].is_string() ? terra["uuid"].get<std::string>() : std::string {};
        ...
        if (!terra.contains("schemaVersion") || !terra["schemaVersion"].is_number_integer() || terra["schemaVersion"] != 1) {
          terra["schemaVersion"] = 1;
          migrate_catalog = true;
        }
        migrate_catalog = normalize_terra_metadata(terra) || migrate_catalog;
        ...
        if (!exclude_global_prep.value_or(false)) {
          prep_cmds.reserve(config::sol.prep_cmds.size());
          for (auto &prep_cmd : config::sol.prep_cmds) {
        ...
        if (terra.contains("legacyId")) {
          persisted_id = parse_legacy_app_id(terra["legacyId"]);
        }
        ...
        if (!terra.contains("legacyId") || parse_legacy_app_id(terra["legacyId"]) != ctx.id) {
          terra["legacyId"] = std::stoi(ctx.id);
          migrate_catalog = true;
        }
        ids.insert(ctx.id);
        ++i;

        ctx.terra_metadata = terra;
      ...
      if (migrate_catalog) {
        if (file_handler::write_file_atomic(file_name.c_str(), source_tree.dump(4)) != 0) {
          throw std::runtime_error("Failed to persist stable Terra application metadata");
        }
      }
