...
namespace video {
  ...
  /**
   * @brief Check whether a Sol colorspace represents HDR video.
   */
  bool colorspace_is_hdr(const sol_colorspace_t &colorspace) {
    return colorspace.colorspace == colorspace_e::bt2020;
  }

  /**
   * @brief Derive Sol colorspace metadata from client stream configuration.
   */
  sol_colorspace_t colorspace_from_client_config(const config_t &config, bool hdr_display) {
    sol_colorspace_t colorspace;
  ...
  /**
   * @brief Convert Sol colorspace metadata to FFmpeg AVCodec fields.
   */
  avcodec_colorspace_t avcodec_colorspace_from_sol_colorspace(const sol_colorspace_t &sol_colorspace) {
    avcodec_colorspace_t avcodec_colorspace;

    switch (sol_colorspace.colorspace) {
      ...
      case colorspace_e::bt2020sdr:
        ...
        assert(sol_colorspace.bit_depth == 10);
    ...
    avcodec_colorspace.range = sol_colorspace.full_range ? AVCOL_RANGE_JPEG : AVCOL_RANGE_MPEG;

    return avcodec_colorspace;
  }

  const color_t *color_vectors_from_colorspace(const sol_colorspace_t &colorspace, bool unorm_output) {
    constexpr auto generate_color_vectors = [](const sol_colorspace_t &colorspace, bool unorm_output) -> color_t {
