...
namespace system_tray {
  ...
  /**
   * @brief Forwards Qt log messages to Sol's BOOST_LOG logger.
  ...
  void tray_quit_cb([[maybe_unused]] struct tray_menu *item) {
    ...
    if (GetConsoleWindow() == nullptr) {
      lifetime::exit_sol(ERROR_SHUTDOWN_IN_PROGRESS, true);
      return;
    }
  #endif
  ...
  static struct tray tray = {
    ...
    .menu =
      (struct tray_menu[]) {
        ...
        {.text = "Open Sol", .cb = tray_open_ui_cb},
        {.text = "-"},
  #ifdef _WIN32
