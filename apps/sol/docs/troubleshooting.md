...
@tabs{
  @tab{General | ```bash
    sol --creds {new-username} {new-password}
    ```
  }
  @tab{AppImage | ```bash
    ./sol.AppImage --creds {new-username} {new-password}
    ```
  }
  @tab{Flatpak | ```bash
    flatpak run --command=sol dev.lizardbyte.app.Sol --creds {new-username} {new-password}
    ```
  }
}
...
If you experience unusual mouse behavior, try attaching a physical mouse to the Sol host.
...
Sol-provided controller (connected to the guest) is the "first" one. In Linux this can be achieved on USB
...
On the Sol host `iperf3` is started in server mode:
...
If the host PC (running Sol) has a much faster connection to the network
than the slowest segment of the network path to the client device (running
Terra), massive packet loss can occur: Sol emits its stream in bursts
...
only Sol's traffic is slowed down.
...
# 4) Create class 1:10 for Sol game stream at 1 Gbit/s
...
In that way only the Sol traffic is limited by 1 Gbit. This is not persistent on reboots.
If you use a different port for the game stream, you need to adjust the last command.

Upstream Sunshine versions > 0.23.1 include improved networking code that should
...
in conjunction with a Sol installation that runs in a sandboxed or reduced permissions state
...
To check if you are affected by this issue, look out for this message in your Sol log:
```
...
> | Desktop Environment | Vulkan Supported? | Recommended Sol Install Type | Recommended Capture & Encoder Configuration       |
> |:--------------------|-------------------|-----------------------------------|--------------------------------------------------:|
...
If you see the above error in the Sol logs, compiling *Mesa* manually may be required. See the official Mesa3D
...
> system. You may also want to enable decoders, however, that is not required for Sol and is not covered here.
> ```bash
...
circumstances (a Sol crash, switching to another desktop environment, or if a monitor hotplug event occurs)
...
configuration to enable permanent capture autorization for Sol via Portal capture:
```
flatpak permission-set kde-authorized remote-desktop dev.lizardbyte.app.Sol yes
```
> [!NOTE]
> Although this configuration is plumbed through Flatpak, it will work with any supported Sol installation type.
...
Sol recreates virtual gamepad device nodes for each streaming session. Manual `chmod` or `setfacl`
changes therefore disappear when the client reconnects. Confirm that the installed Sol rule contains the
...
your compositor may ignore injected input unless Sol's virtual devices are assigned to the correct seat.

Sol determines its target seat from `XDG_SEAT`, which is typically set automatically by your display manager.
If needed, you can override it manually in your systemd service file or shell environment before starting Sol.

When the seat is not `seat0`, Sol appends the seat name to its virtual device names, for example:

- Keyboard passthrough (seat1)
- Sol (libvirtualhid) PS5 Controller (seat1)

Sol creates two mouse devices: a relative one and an absolute one.

To assign Sol's virtual devices to the correct seat, create this udev rules file
(/etc/udev/rules.d/72-sol-virtual-seat.rules):
```udev
...
This means that you must install Sol using the native package format of your distribution, if available.
...
If you notice unexpectedly high encoding latencies (e.g., in Terra's
...
```
Sol sets this variable automatically, no manual
...
### No gamepad detected
Sol uses libvirtualhid for virtual input on Windows. Install the
...
Sol requires Virtual HID Driver version `2026.829.2338.54` or newer. Earlier releases use incompatible Windows
...
An active Virtual HID Driver machine license is required before Sol can create driver-backed libvirtualhid
...
With a compatible Virtual HID Driver and active license, Sol sends normal key transitions through a real HID
...
Check the Virtual HID Driver version and license sections on the Web UI Troubleshooting page. Sol recreates the
shared keyboard and mouse after a successful license activation, validation, or deactivation, so you do not need to
restart Sol merely to switch between the HID and SendInput paths.

### Games do not detect mouse input
With a compatible Virtual HID Driver and active license, Sol sends relative mouse movement, buttons, and scrolling
...
requiring a Sol restart.

### Permission denied
Since Sol runs as a service on Windows, it may not have the same level of access that your regular user account
