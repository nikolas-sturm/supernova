...
A Vite dev server with hot module replacement and a proxy to the local Sol
backend is available. Start Sol, then run:
...
### Localization
Sol and related LizardByte projects are being localized into various languages.
...
##### Translation Basics
* The brand names *LizardByte* and *Sol* should never be translated.
* Other brand names should never be translated. Examples include *AMD*, *Intel*, and *NVIDIA*.

##### CrowdIn Integration
How does it work?

When a change is made to Sol source code, a workflow generates new translation templates
...
##### Web UI
Sol uses [i18next](https://www.i18next.com) with [react-i18next](https://react.i18next.com)
...
* Add the string to the `./src_assets/common/assets/web/public/assets/locale/en.json` file, in English.
  ...
  > to merge the translations into Sol.
...
uv are required for this, along with the Python dependencies in the Sol `pyproject.toml`. From the repository
...
> complete, a PR will be made to merge the translations into Sol.
...
#### Unit Testing
Sol uses [Google Test](https://github.com/google/googletest) for unit testing. Google Test is included in the
