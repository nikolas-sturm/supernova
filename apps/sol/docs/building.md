# Building
Sol binaries are built using [CMake](https://cmake.org) and requires `cmake` > 3.25.
...
#### FreeBSD
> [!CAUTION]
> Sol support for FreeBSD is experimental and may be incomplete or not work as expected
...
##### KMS Capture
If you are using KMS, patching the Sol binary with `setcap` is required. Some post-install scripts handle this. If building
from source and using the binary directly, this will also work:

```bash
sudo cp build/sol /tmp
sudo setcap cap_sys_admin,cap_sys_nice+p /tmp/sol
sudo getcap /tmp/sol
sudo mv /tmp/sol build/sol
```

##### CUDA Toolkit
Sol requires CUDA Toolkit for NVFBC capture. There are two caveats to CUDA:
...
Static Qt is enabled by default on Windows. Sol automatically adds the MSYS2 static Qt prefix at
`${MINGW_PREFIX}/qt6-static` when that package is installed. If an IDE does not inherit `MINGW_PREFIX`, Sol
...
To use dynamic Qt instead, configure with `-DSOL_USE_STATIC_QT=OFF` and ensure the dynamic Qt package is
...
### Clone
For Sol in this workspace, use the shared `tooling/native/build.mjs` entry point
from the workspace root, with separate `cmake-build-` trees for each platform and
configuration. See [naming and required workspace updates](sol.md).

The following clone example requires [git](https://git-scm.com) and refers to the
original upstream Sunshine repository, not this workspace:
