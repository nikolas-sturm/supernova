...
> [!TIP]
> Replace ``HDMI-A-1`` with the display name of the monitor you would like to use for Terra.
...
> ``do`` command to fetch the resolution requested by your Terra client
...
##### Windows
Sol has built-in support for changing the resolution and refresh rate on Windows. If you prefer to use a
...
> host, the Flatpak of Sol requires commands to be prefixed with `flatpak-spawn --host`.

#### Windows
**Elevating Commands (Windows)**

If you've installed Sol as a service (default), you can specify if a command should be elevated with
