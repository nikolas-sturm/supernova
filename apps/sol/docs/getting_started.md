...
## Sol in This Workspace

Sol is built from `apps/sol` for Windows and Linux. The workspace project
is named `sol`. The native executable is `sol` (`sol.exe` on Windows),
the Windows service wrapper is `tools/solsvc.exe`, and the test binary is
`tests/test_sol`. Linux packages install `sol.service`, also available as
`app-dev.lizardbyte.app.Sol.service`.

Use locally produced Sol packages rather than treating the upstream links below as
Sol downloads. See [Sol naming and compatibility](sol.md) before moving an existing
installation. Initial setup below uses Sol names on supported platforms.

## Upstream Sunshine Distribution Reference

The distribution instructions through the end of the Binaries section are retained
upstream Sunshine references. Their repository names, package names, downloads, and
unsupported-platform instructions do not describe Sol releases.
...
**Start once**
```bash
systemctl --user start app-dev.lizardbyte.app.Sol
```

**Start on boot**
```bash
systemctl --user --now enable app-dev.lizardbyte.app.Sol
```

> [!NOTE]
> The service has been renamed to "app-dev.lizardbyte.app.Sol" in order to increase compatibility with
> XDG Desktop Portal, but it is also aliased to "sol.service" for convenience.
...
### Windows
Sol uses libvirtualhid for virtual input on Windows. You must install the
...
control and broker protocols and must be upgraded together with Sol's embedded libvirtualhid library. Local
...
Input keyboard and mouse. Sol shows the current license status and actions on the Web UI Troubleshooting page and
in the **Virtual HID Driver** system tray submenu. When Sol starts on an unactivated machine, select its tray
notification to open the activation and purchase options in the Web UI. Sol recreates the shared keyboard and
...
### Basic usage
If Sol is not installed/running as a service, then start Sol with the following command, unless a start
command is listed in the specified package [install](#install) instructions above.

> [!NOTE]
> A service is a process that runs in the background. This is the default when installing Sol from the
> Windows installer. Running multiple instances of Sol is not advised.
...
### Specify config file
```bash
sol <directory of conf file>/sunshine.conf
```
...
### Start Sol over SSH (Linux/X11)
Assuming you are already logged into the host, you can use this command

```bash
ssh <user>@<ip_address> 'export DISPLAY=:0; sol'
```

If you are logged into the host with only a tty (teletypewriter), you can use `startx` to start the X server prior to
executing Sol. You nay need to add `sleep` between `startx` and `sol` to allow more time for the display to
be ready.

```bash
ssh <user>@<ip_address> 'startx &; export DISPLAY=:0; sol'
```
...
Sol is configured via the web ui, which is available on [https://localhost:47990](https://localhost:47990)
...
4. Find Terra clients and other tools for Sol in the `Featured Apps` tab.
   ![Featured Apps](images/featured-apps.png)
5. In Terra, you may need to add the PC manually.
6. When Terra requests for you insert the pin:

   - Login to the web-ui
   - Go to "PIN" in the Navbar
   - Type in your PIN and press `Enter`, and enter a name of your choosing for the device.
     You should get a Success Message!
   - In Terra, select one of the Applications listed
...
@tabs{
   ...
   @tab{ AppImage | ```bash
      ...
      ```}
   @tab{ Flatpak | ```bash
      flatpak run --command=sol dev.lizardbyte.app.Sol --help
      ```}
}

### Shortcuts
All shortcuts start with `Ctrl+Alt+Shift`, just like Terra.

* `Ctrl+Alt+Shift+N`: Hide/Unhide the cursor (This may be useful for Remote Desktop Mode for Terra)
...
* `env` - Adds or overwrites Environment variables for the commands/applications run by Sol.
  This can only be changed by modifying the `apps.json` file directly.

### Considerations
* On Windows, Sol uses the Desktop Duplication API which only supports capturing from the GPU used for display.
...
* If inputs (mouse, keyboard, gamepads...) aren't working after connecting:
  ...
  * On FreeBSD/Linux, add the user running sol to the `input` group.

* The FreeBSD version of Sol is missing some features that are present on Linux.
...
* General HDR support information and requirements:
  ...
  * You must also enable the HDR option in your Terra client settings, otherwise the stream will be SDR
