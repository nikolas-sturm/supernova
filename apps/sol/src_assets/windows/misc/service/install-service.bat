...
set SERVICE_NAME=SolService
set "SERVICE_BIN=%ROOT_DIR%\tools\solsvc.exe"
...
rem Remove the previous SunshineService before starting its renamed replacement.
net stop SunshineService
sc delete SunshineService

rem Check if SolService already exists
...
sc %SC_CMD% %SERVICE_NAME% binPath= "\"%SERVICE_BIN%\"" start= %SERVICE_START_TYPE% DisplayName= "Sol Service"

rem Set the description of the service
sc description %SERVICE_NAME% "Sol is a self-hosted game stream host for Moonlight."
