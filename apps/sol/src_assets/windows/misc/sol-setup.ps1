# Sol Setup Script
# This script orchestrates the installation and uninstallation of Sol
# Usage: sol-setup.ps1 -Action [install|uninstall] [-Silent]
...
if (-not $Action) {
    Write-Information ""
    Write-FramedText -Message "🔅 Sol Setup Script" -Level "Information" -Color "Cyan"
    Write-Information ""
    Write-LogMessage -Message "Please select an action:" -Level "Information" -Color "Yellow"
    Write-LogMessage -Message "  1. Install Sol" -Level "Information" -Color "Green"
    Write-LogMessage -Message "  2. Uninstall Sol" -Level "Information" -Color "Red"
    Write-Information ""
...
$logDir = Join-Path $env:TEMP "Sol\logs\$Action"
...
# Function to execute sol.exe with arguments if it exists
function Invoke-SolIfExist {
    param(
        [string]$Arguments,
        [string]$Description = "",
        [string]$Emoji = "🔧"
    ...
    $SolPath = Join-Path $RootDir "sol.exe"

    if (Test-Path $SolPath) {
        Write-LogMessage -Message "Executing: $SolPath $Arguments" -Level "Information"
        ...
        try {
            ...
            if ($process.ExitCode -ne 0) {
                Write-LogMessage -Message "  ⚠ Sol exited with code $($process.ExitCode)" -Level "Warning"
                return $process.ExitCode
            } else {
                Write-LogMessage -Message "  ✓ Done" -Level "Success"
                return 0
...
if ($Action -eq "install") {
    Write-FramedText `
        -Message "🔅 Sol Installation Script" `
        -Level "Information" `
        -Color "Yellow"
    Write-Information ""
    ...
    Invoke-ScriptIfExist `
        ...
        -Arguments "add" `
        -Description "Adding Sol directories to PATH" `
        -Emoji "📁"
    Write-Information ""
    ...
    Write-Progress -Activity "Installing Sol" -Completed
    Write-FramedText -Message "✓ Sol installation completed successfully!" -Level "Success"
    ...
    Write-FramedText `
        -Message "🗑️  Sol Uninstallation Script" `
        -Level "Information" `
        -Color "Yellow"
    Write-Information ""
    ...
    Write-Progress -Activity "Uninstalling Sol" -Completed
    Write-FramedText `
        -Message "✓ Sol uninstallation completed successfully!" `
        -Level "Success"
}
