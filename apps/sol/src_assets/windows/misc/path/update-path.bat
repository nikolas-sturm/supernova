...
if "%~1"=="" (
    ...
    echo   add    - Adds Sol directories to system PATH
    echo   remove - Removes Sol directories from system PATH
    exit /b 1
)

rem Get sol root directory
for %%I in ("%~dp0\..") do set "ROOT_DIR=%%~fI"
echo Sol root directory: !ROOT_DIR!
...
if /i "%~1"=="add" (
    ...
    if "!NEW_PATH!" neq "!CURRENT_PATH!" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully added Sol directories to PATH
        ) else (
            echo Failed to add Sol directories to PATH
        )
    ) else (
...
if /i "%~1"=="remove" (
    ...
    if "!CHANGES_MADE!"=="1" (
        ...
        if !ERRORLEVEL!==0 (
            echo Successfully removed Sol directories from PATH
        ) else (
            echo Failed to remove Sol directories from PATH
        )
    ) else (
        echo No changes needed to PATH
    )
