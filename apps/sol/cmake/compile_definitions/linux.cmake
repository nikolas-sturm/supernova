...
if(FREEBSD)
    add_compile_definitions(SOL_PLATFORM="freebsd")
    ...
    # so all targets (sol, test_sol) can resolve libraries found via pkg_check_modules.
    link_directories(/usr/local/lib)
else()
    add_compile_definitions(SOL_PLATFORM="linux")
endif()

# AppImage
if(${SOL_BUILD_APPIMAGE})
    # use relative assets path for AppImage
    string(REPLACE "${CMAKE_INSTALL_PREFIX}" ".${CMAKE_INSTALL_PREFIX}" SOL_ASSETS_DIR_DEF ${SOL_ASSETS_DIR})
endif()
...
if(${SOL_ENABLE_CUDA})
    ...
    if(CMAKE_CUDA_COMPILER)
        ...
        if(CMAKE_CUDA_COMPILER_VERSION VERSION_GREATER_EQUAL 12.0)
            ...
            message(FATAL_ERROR
                    "Sol requires a minimum CUDA Compiler version of 12.0.
    ...
    elseif(${CUDA_FAIL_ON_MISSING})
        message(FATAL_ERROR
                "CUDA not found.
                If this is intentional, set '-DSOL_ENABLE_CUDA=OFF' or '-DCUDA_FAIL_ON_MISSING=OFF'"
        )
    endif()
endif()
if(CUDA_FOUND)
    ...
    add_compile_definitions(SOL_BUILD_CUDA)
endif()

# libdrm is required for DRM (KMS). Only the headers are required for Wayland,
# Vulkan, and PipeWire (KWin, Portal).
if(${SOL_ENABLE_DRM} OR ${SOL_ENABLE_WAYLAND} OR ${SOL_ENABLE_VULKAN}
   OR ${SOL_ENABLE_KWIN} OR ${SOL_ENABLE_PORTAL})
...
if(LIBDRM_FOUND)
    ...
    if(${SOL_ENABLE_DRM})
        ...
        add_compile_definitions(SOL_BUILD_DRM)
        list(APPEND PLATFORM_TARGET_FILES
                "${CMAKE_SOURCE_DIR}/src/platform/linux/kmsgrab.cpp")
        list(APPEND SOL_DEFINITIONS EGL_NO_X11=1)
    endif()
endif()
...
# evdev
include(dependencies/libevdev_Sol)
...
if(LIBVA_FOUND)
    add_compile_definitions(SOL_BUILD_VAAPI)
...
if(${SOL_ENABLE_VULKAN})
    if(NOT SOL_SYSTEM_VULKAN_HEADERS)
    ...
    list(APPEND SOL_DEFINITIONS SOL_BUILD_VULKAN=1)
    ...
    set(VULKAN_SHADER_SOURCE "${SOL_SOURCE_ASSETS_DIR}/linux/assets/shaders/vulkan/rgb2yuv.comp")
    ...
    set(SOL_TARGET_DEPENDENCIES ${SOL_TARGET_DEPENDENCIES} vulkan_shaders)
endif()

# wayland
if(${SOL_ENABLE_WAYLAND})
    find_package(Wayland REQUIRED)
else()
    set(WAYLAND_FOUND OFF)
endif()
if(WAYLAND_FOUND)
    add_compile_definitions(SOL_BUILD_WAYLAND)

    if(NOT SOL_SYSTEM_WAYLAND_PROTOCOLS)
...
if(X11_FOUND)
    add_compile_definitions(SOL_BUILD_X11)
...
# Pipewire
if(${SOL_ENABLE_KWIN} OR ${SOL_ENABLE_PORTAL})
...
if(PIPEWIRE_FOUND AND GIO_FOUND AND ${SOL_ENABLE_PORTAL})
    set(PORTAL_FOUND ON)
    add_compile_definitions(SOL_BUILD_PORTAL)
    list(APPEND PLATFORM_TARGET_FILES
...
set(KWIN_FOUND OFF)
if(PIPEWIRE_FOUND AND WAYLAND_FOUND AND ${SOL_ENABLE_KWIN})
    set(KWIN_FOUND ON)
    add_compile_definitions(SOL_BUILD_KWIN)
...
elseif(${SOL_ENABLE_KWIN} AND NOT WAYLAND_FOUND)
    message(FATAL_ERROR "SOL_ENABLE_KWIN requires SOL_ENABLE_WAYLAND — KWin capture disabled")
endif()
...
if (${SOL_BUILD_APPIMAGE})
    list(APPEND SOL_DEFINITIONS SOL_BUILD_APPIMAGE=1)
endif ()
if (${SOL_BUILD_FLATPAK})
    list(APPEND SOL_DEFINITIONS SOL_BUILD_FLATPAK=1)
endif ()
...
list(APPEND SOL_EXTERNAL_LIBRARIES glad)
