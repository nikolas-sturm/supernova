...
include(dependencies/Boost_Sol)
...
if(SOL_ENABLE_TRAY)
    if(SOL_USE_STATIC_QT)
        set(_sol_find_library_suffixes "${CMAKE_FIND_LIBRARY_SUFFIXES}")
        set(_sol_import_library_suffix "${CMAKE_IMPORT_LIBRARY_SUFFIX}")
        set(_sol_pkg_config_argn "${PKG_CONFIG_ARGN}")
        set(_sol_disable_find_package_harfbuzz "${CMAKE_DISABLE_FIND_PACKAGE_harfbuzz}")
        ...
        pkg_check_modules(SOL_STATIC_TIFF REQUIRED IMPORTED_TARGET libtiff-4)
        add_library(TIFF::TIFF INTERFACE IMPORTED)
        target_link_libraries(TIFF::TIFF INTERFACE PkgConfig::SOL_STATIC_TIFF)

        set(_sol_module_path "${CMAKE_MODULE_PATH}")
        find_package(Qt6 REQUIRED COMPONENTS Widgets Svg)
        get_target_property(_sol_qt_core_type Qt6::Core TYPE)
        if(NOT _sol_qt_core_type STREQUAL "STATIC_LIBRARY")
            message(FATAL_ERROR "SOL_USE_STATIC_QT requires a static Qt 6 installation.")
        endif()
        set(CMAKE_MODULE_PATH "${_sol_module_path}")
    endif()

    add_subdirectory("${CMAKE_SOURCE_DIR}/third-party/tray")

    if(SOL_USE_STATIC_QT)
        set(CMAKE_FIND_LIBRARY_SUFFIXES "${_sol_find_library_suffixes}")
        set(CMAKE_IMPORT_LIBRARY_SUFFIX "${_sol_import_library_suffix}")
        set(PKG_CONFIG_ARGN "${_sol_pkg_config_argn}")
        set(CMAKE_DISABLE_FIND_PACKAGE_harfbuzz "${_sol_disable_find_package_harfbuzz}")
        unset(_sol_disable_find_package_harfbuzz)
        unset(_sol_find_library_suffixes)
        unset(_sol_import_library_suffix)
        unset(_sol_module_path)
        unset(_sol_pkg_config_argn)
        unset(_sol_qt_core_type)
    endif()
endif()
