...
Options:
  ...
  --publisher-issue-url    The URL of the publisher's support site or issue tracker.
                           If you provide a modified version of Sol, we kindly request that you use your own url.
...
function run_step_deps() {
  ...
  if ! check_version "${DOXYGEN}" "$doxygen_min" "$doxygen_max"; then
    if [[ "${SOL_COMPILE_DOXYGEN}" == "true" ]]; then
...
function run_step_cmake() {
  ...
  cmake_args=(
    ...
    "-DSOL_ASSETS_DIR=share/sol"
    "-DSOL_EXECUTABLE_PATH=/usr/bin/sol"
    "-DSOL_ENABLE_DRM=ON"
    "-DSOL_ENABLE_KWIN=ON"
  ...
  if [[ "$appimage_build" == 1 ]]; then
    cmake_args+=("-DSOL_BUILD_APPIMAGE=ON")
  fi

  # Publisher metadata
  if [[ -n "$publisher_name" ]]; then
    cmake_args+=("-DSOL_PUBLISHER_NAME='${publisher_name}'")
  fi
  if [[ -n "$publisher_website" ]]; then
    cmake_args+=("-DSOL_PUBLISHER_WEBSITE='${publisher_website}'")
  fi
  if [[ -n "$publisher_issue_url" ]]; then
    cmake_args+=("-DSOL_PUBLISHER_ISSUE_URL='${publisher_issue_url}'")
  fi

  # Handle doxygen docs flag
  if ! check_version "${DOXYGEN}" "$doxygen_min" "$doxygen_max" && [[ "${SOL_COMPILE_DOXYGEN}" != "true" ]]; then
...
function run_step_validation() {
  ...
  appstreamcli validate "build/dev.lizardbyte.app.Sol.metainfo.xml"
  appstream-util validate "build/dev.lizardbyte.app.Sol.metainfo.xml"
  desktop-file-validate "build/dev.lizardbyte.app.Sol.desktop"
  if [[ "$appimage_build" == 0 ]]; then
    desktop-file-validate "build/dev.lizardbyte.app.Sol.terminal.desktop"
  fi
  return 0
}
