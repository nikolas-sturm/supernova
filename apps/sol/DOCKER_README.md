...
The `docker/` files in this workspace build Sol containers with `/usr/bin/sol` as
their entry point. The published images and tags described below belong to upstream
Sunshine; they do not contain a `sol` executable.

The upstream image provides a method for you to use the latest Sunshine release in your own docker projects. It is not
...
### SUNSHINE_OS
Upstream Sunshine images are available with the following tag suffixes, based on their respective base images.
...
## Where used
This is a list of docker projects using upstream Sunshine.
...
| `-v <path to data>:/config` | Volume mapping       | `/home/sol`   | True     |
